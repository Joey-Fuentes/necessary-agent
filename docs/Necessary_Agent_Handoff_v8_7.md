# A Necessary Agent — Handoff (v8.7)
...
**Verified:** `NecessaryAgent.lean` compiles under Lean 4.33.1 (core, no Mathlib), exit 0, zero errors, zero warnings, zero `sorry`; 42 `#print axioms` results, every one depending on at most `propext`, `choice`, `Quot.sound`. Output in `NecessaryAgent_v8_7_print_axioms.txt`. MD5 of the verified file: `21290ecf4e49ecf77894593cb036cab9`.
...
1. **The TR fork (D23).** Formalize `Directed s D` as a primitive distinct from `Sens`; certify `Directed ∧ ¬Sens` consistent; argue (in prose, not by axiom) that `Directed` with a discriminating D does not yield `Sens`, so the powers naturalist must choose: alternatives figure (⇒ TR via Oderberg's abstraction) or they don't (⇒ `W_Nat`). The nil alternative is the lever. Answer the Dretskean modus tollens explicitly. If a referee produces a field-sensitivity-without-abstraction model, the conclusion weakens from "TR is forced" to "TR is the most economical reading" and must say so. Done = a new witness and a TR dossier section.
