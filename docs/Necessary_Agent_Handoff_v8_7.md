# A Necessary Agent — Handoff (v8.8)
...
**Verified:** `NecessaryAgent.lean` compiles under Lean 4.33.1 (core, no Mathlib), exit 0, zero errors, zero warnings, zero `sorry`; 47 `#print axioms` results, every one depending on at most `propext`, `choice`, `Quot.sound`. CI (`scripts/verify.sh`) passes locally on this commit. Output in `NecessaryAgent_v8_7_print_axioms.txt`. MD5 of the verified file: `21290ecf4e49ecf77894593cb036cab9`.
...
1. **The TR fork — one position left (D23; III.5).** Certified in v8.8 on the powers theorist's own primitive (`Powers M`: `Directed s D` + `manif`): `powers_fork`, `powers_all_tied` (no axioms), `W_Nat.powers_witness` (Horn 2 is the bare naturalist), `NoTR.powers_horn1` (Horn 1 consistent only when every reality is tied), `W_Pref.no_powers_at_maximal` (the theist's state is not a power). What remains: the fallible reading — an inclination toward the best that may fail at w′ — is not a power and is `Sens`-as-inclination; the burden is Oderberg's abstraction criterion applied to it, which is philosophical, not formal. Next concrete step: a III.5 subsection that states the fallible reading in the powers theorist's own vocabulary (Vetter's potentiality; Mumford–Anjum's tendencies) and argues that a tendency individuated by "whichever is best among the alternatives" has no determinate manifestation-type and so is not a tendency on their account either. Threshold: a published account of a fallible, field-ranging, non-mental inclination reopens the exit.
