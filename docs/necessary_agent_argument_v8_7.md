...
**Changes from v8.7 (this revision — v8.8: the TR fork certified).**
...
0. **The TR fork — narrowed** (D23; III.5). Certified in v8.8 on the powers theorist's own primitive: `powers_fork`, `powers_all_tied`, `W_Nat.powers_witness`, `NoTR.powers_horn1`, `W_Pref.no_powers_at_maximal`. What remains is the fallible reading: a directedness at maximality that may fail at w′, which is not a power (`manif` fails) and is `Sens`-as-inclination. The residual philosophical burden is Oderberg's criterion applied — that an inclination ranging over unproduced alternatives, toward whichever is best, exhibits specific indifference and is therefore mental. No theorem does that step; a referee who supplies a non-mental account of fallible field-ranging inclination without a power reopens the exit, and the conclusion then weakens to "TR is the most economical reading." Still the argument's central burden, but the exit is now one position wide.
...
| TR | **The logical load of the argument.** Certified independent by a mindless `NoTR` (v8.6). The fork is certified (v8.8, `powers_fork`): on the powers theorist's own primitive and axiom, denying TR is either the bare naturalist (`W_Nat.powers_witness`) or a world in which every reality is tied (`powers_all_tied`); the theist's state is not a power (`W_Pref.no_powers_at_maximal`). What remains is the fallible-inclination reading, one position wide | Oderberg's abstraction criterion applied to a fallible field-ranging inclination (open problem 0) |
...
*What remains.* The powers naturalist's remaining position is the *fallible* one: a directedness at maximality that, like `Sens`, may fail at w′. By `W_Pref.no_powers_at_maximal` that is not a power on any `Powers` structure — it violates `manif` — so the powers theorist who takes it has dropped the power and kept only the directedness. What he then has is a state whose contribution inclines toward the best among the alternatives and may fail to prevail: that is `Sens`-as-inclination exactly, and it is the reading on which the alternatives *figure* — the inclination is toward one among them, and the failure world is one where a non-maximal candidate was produced despite it. That is Oderberg's specific indifference — directedness at "whichever is best," ranging over instances — which he reserves for the mental and says physical powers have no correlate of. TR's question returns here in full, and here it is no longer "why ponens rather than tollens" but "what is a fallible inclination toward the best, ranging over alternatives, that is about none of them?" That is the residual burden, and it is a burden of the powers theorist's, not the argument's: he has abandoned his own primitive to reach it.
...
To deny TR is to hold that a state of the necessary ground through which the first production is sensitive to a property of the candidate realities, and in virtue of which the ground produces, has no alternative as content — including the alternative of producing nothing. `Toy.NoTR` certifies the position is consistent and mindless. Its price is now certified as a fork (`powers_fork`): on the powers theorist's own primitive and own axiom, the channel is either directed at a determinate type — in which case it is the bare naturalist's channel (`W_Nat.powers_witness`), CE fails, the channel does not track the good, and the contrast among the D-realities is bare — or directed at maximality, in which case every possible reality is tied on the balance (`powers_all_tied`), the balance discriminates nothing, and "something mattered" has collapsed into a property every candidate has. The one position between the horns — a fallible directedness at the best — is not a power (`W_Pref.no_powers_at_maximal`) and is `Sens`-as-inclination, where the alternatives figure and the question "about none of them?" is the powers theorist's to answer with a primitive he no longer has.

### Ceiling

TR cannot be proved against a theorist who insists that a fallible, field-ranging inclination toward the best is directedness without content. What can be shown, and now is: that the powers theorist's own primitive, under his own axiom, yields exactly two positions — the bare naturalist (`W_Nat.powers_witness`) and the tied world (`powers_all_tied`, `NoTR.powers_horn1`) — and that the discriminating balance the theist has is one no power can have (`W_Pref.no_powers_at_maximal`); that TR asks for indicator content only; that the nil alternative is one no determinate-type power is directed at; and that the Dretskean modus tollens is available only on Horn 2, which is ¬CE. What cannot be shown by theorem is that a fallible inclination ranging over alternatives is *about* them — that step is Oderberg's abstraction criterion, applied, and it is philosophical. The honest statement of where this leaves the powers naturalist: he has an exit only if he can say what a fallible, field-ranging inclination toward the best is, without a power and without content. That is narrower than v8.7's ceiling and it is the burden that remains in §10.
...
### A.5 The TR fork (v8.8): `Powers`, `PowersChannel`, `powers_all_tied`, `powers_fork`
