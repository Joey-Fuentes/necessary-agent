...
## Version 8.14 — after the third external review: the fallible reading reopened, the two-fundamental positions certified, the uniform propensity horn certified, statements pinned in CI. Machine-checked core; P3, P6 and P10 derived; the necessary-mind conclusion needs no value premise; P4⁺ adopted: one being; soundness dossier complete; two naturalist packages certified (¬CE and ¬TR); the theist's residue certified and priced; the conclusion named (`God`) and certified (`god_exists`); the TR fork certified (`powers_fork`) and the fallible reading finished (`FalliblePowers`); P4 narrowed to uniqueness (`CoreNoP4`, `W_P4.two_minds`); III.4 rewritten; IDF stated and its limit certified; CatU, P1, P11 argued; 58 certified results
...
**Changes from v8.13 (this revision — v8.14: after the third external review, which built its attacks as compiling countermodels; every one is now in the file).**
...
- (bv) **IDF stated in the file.** `SameProfile t t'` (matched necessary states with the same content, mentality, accuracy, sensitivity) and `IDF_stmt` (fundamentals sharing every necessary feature are one) are defined as an optional premise. It is anti-haecceitism about the foundation, made once, and it is what P4's uniqueness and CE's reply to the haecceitistic exit both rest on (D25).
...
- (bp) **III.6 P4 rewritten.** The coordination argument developed as a trilemma for the second being — bare source (Exit 1's bill for its share), field-sensitive (a second mind), or mediated (P4⁺'s territory) — and uniqueness located at the premise it actually rests on: IDF, identity of indiscernible fundamentals, which is anti-haecceitism at the foundation and the same commitment CE already needs against Objection 9. Island universes, the aggregate move, and the literature (Pruss & Rasmussen bracketing uniqueness; Rasmussen's pure actuality; Leftow's GSA; Gellman; Swinburne; Mghirbi's concession) placed. Open problem 6 is closed to the extent formal work can close it; what remains is IDF's defense, which is III.1's.
...
- **D25** IDF (v8.12). `SameProfile` and `IDF_stmt` are defined in the file as an optional premise: fundamental beings sharing every necessary feature are one. It is the refusal of primitive thisness at the foundation, made once, on which both P4's uniqueness (III.6) and CE's reply to the haecceitistic exit (III.1, Obj. 9) rest. `W_P4.IDF_and_not_P4` certifies that IDF alone does not yield P4: the alternation model — each fundamental active at a different world — satisfies IDF vacuously and violates P4. Uniqueness is therefore IDF plus CE's pricing of bare sources, and the file says so.
...
| P4 | `W_P4` — N with s1 produces c1 at `true`; N2 with s2 produces c2 at `false` | two fundamental beings | 2.8: one being brings about every reality — but `W_P4.two_minds`: both beings are necessary minds. `W_P4` is III.6's position (iv), alternation; positions (i) and (ii) are `W_Two` (v8.14, rows below) |
...
| P4 | Used only for uniqueness and identification (`CoreNoP4`, v8.10); the mind conclusion survives its denial and `W_P4.two_minds` shows the denial is polytheism; uniqueness rests on IDF (anti-haecceitism at the foundation), the commitment CE already needs; the flagship literature brackets uniqueness and Mghirbi (2026) concedes it needs an extra premise — the dossier names it | A defense of IDF (= III.1's Objection 9) and parsimony against primitive thisness |
...
### A.6 v8.14 additions: `naturalPowers`, `fallible_actual_iff`, the uniform horn
