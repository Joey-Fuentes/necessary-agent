...
0. **TR — the fallible reading, reopened (v8.14)** (D23, D26; III.5). Certified: `powers_fork`, `powers_all_tied`, `no_powers_at_maximal` close the *necessitating* reading. Withdrawn: the v8.9 claim that `W_Nat.fallible_witness`/`W_Pref.fallible_witness` show "the same structure both sides" — `fallible_actual_iff` certifies the structure is content-free, and the witnesses are deleted. `W_Pref.fork_is_residue` shows Horn 1 is the theist's residue. What remains: a fallible directedness has no formalization; the powers naturalist holds the theist's modal primitive and denies TR's content claim; III.5's readings of Mumford–Anjum, Vetter and Bird are corrected to the texts. Open, and the largest item on the list.
...
6. **P4 — narrowed (v8.10), positions certified (v8.14).** The mind conclusion does not use P4 (`mind_of_CE_NBL` over `CoreNoP4`). Position (ii) — a second channel-bearing fundamental — is `W_Two.Both.position_ii` and is excluded by IDF alone. Positions (i) and (iv) — a second fundamental that produces with no channel (`W_Two.OneChannel.position_i`), or alternation (`W_P4`) — are consistent with IDF, CE, and every field but P4/P4⁺, with **no world `Bare`**: the model has no price for them. What remains: a per-being notion of contribution to F_w, without which the "bare source" of III.6 (i) is unpriced; and IDF's own defense, separate from ID's.
...
17. **IDF** (v8.12; scoped v8.14). Stated as an optional premise (III.6). Certified: not sufficient for P4 (`W_P4.IDF_and_not_P4`); sufficient against position (ii) (`W_Two.Both.not_IDF` is what excludes it); silent on (i) and (iv). IDF and ID are about different sorts (`Thing` vs `Item`) and neither is bought by the other; `W_P4` has IDF and ¬ID. Remaining: IDF's defense against a foundation-haecceitist, on its own.
...
| P4 | Used only for uniqueness and identification (`CoreNoP4`, v8.10); the mind conclusion survives its denial. Uniqueness against a second channel-bearing fundamental rests on IDF (`W_Two.Both.not_IDF`, v8.14), a commitment separate from CE's Objection 9 (`W_P4.IDF_and_not_ID`, v8.15); against a second channel-less producer (`W_Two.OneChannel.position_i`) or alternation (`W_P4`) the model has no price. The flagship literature brackets uniqueness and Mghirbi (2026) concedes it needs a premise beyond modal logic | IDF's defense; a per-being notion of contribution to F_w |
...
| TR | **The logical load of the argument.** Certified independent by a mindless `NoTR` (v8.6). The *necessitating* reading is closed (v8.8, `powers_fork`, `powers_all_tied`, `no_powers_at_maximal`). The fallible reading is **open** (v8.14): `fallible_actual_iff` shows the v8.9 structure was content-free and its witnesses are deleted; `powers_fork`'s Horn 1 is the theist's residue (`W_Pref.fork_is_residue`); the Mumford–Anjum/Vetter/Bird closures do not hold to the texts (III.5). What remains is TR's content claim against a powers naturalist holding the theist's own modal primitive | The case for TR (III.5 (a)–(c)) and the Oderberg step under the field-ranging reading; a formalization of fallible directedness with content about the state |
...
TR cannot be proved against a theorist who asserts, as a primitive, a fallible inclination toward the best among alternatives that is about none of them. What can be shown, and is: that a *necessitating* powers channel ties every reality or fails on a discriminating balance (`powers_fork`, `powers_all_tied`, `no_powers_at_maximal`). What cannot be shown, and was asserted in v8.9–8.13: that a fallible directedness is "not a tendency, potentiality, or disposition" (the texts do not say so); that it is "the same structure both sides" (`fallible_actual_iff`: nothing with content was defined); or that its holder owes one sentence — he owes the same modal primitive the theist holds, and the dispute is entirely TR's content claim, on which the powers literature is silent. The powers naturalist's exit is therefore TR, whole; its price is the case for TR in (a)–(c), the Oderberg step under the field-ranging reading, and nothing certified.
...
**Cost of denial (v8.14).** Position (ii): several necessary concrete minds with the same necessary profile, distinguished by primitive thisness alone — `W_Two.Both.position_ii`, excluded by IDF. Positions (i) and (iv): a second fundamental that produces with no channel state (`W_Two.OneChannel.position_i`) or by turns (`W_P4`) — consistent with IDF, CE, and every field but P4/P4⁺, with no world `Bare`; the model has **no price** for these, and the dossier now says so rather than charging "Exit 1's bill for its share." `W_P4.two_minds` certifies that in the alternation model both beings are minds; that is a fact about `Mental := True` in that model, not a theorem that a channel-less second fundamental is a mind.

**Ceiling (v8.14).** Uniqueness is not a theorem of logic. Against a second *channel-bearing* fundamental it rests on IDF, certified to be what excludes that position and to be a commitment separate from ID. Against a second fundamental that produces with no channel it rests on nothing the file contains: `Bare` is world-indexed, and a per-being notion of contribution to F_w — the one piece of formal work left on P4 — is what would let (i) and (iv) be priced. Until it exists, "denying P4 is polytheism" is true of (ii) and false of (i)/(iv), and the argument's claim over a naturalist who posits one brute producer alongside one mind is IDF-independent parsimony, honestly labeled as such.
...
| TR | That a fallible field-ranging inclination toward the best is content rather than a primitive modality. **The fallible reading is not formalized** (`fallible_actual_iff`, v8.14); the v8.9 "same structure both sides" is withdrawn | Certified: `manif` ties everything (`powers_all_tied`) or fails on a discriminating balance (`no_powers_at_maximal`). Not certified: anything about a directedness weaker than `manif`. `powers_fork`'s Horn 1 is `W_Pref.residue` (`fork_is_residue`). The Mumford–Anjum/Vetter/Bird closures do not hold to the texts; the powers naturalist holds the theist's modal primitive and denies TR's content claim — one premise, TR, priced by (a)–(c) and the Oderberg step under the reading Horn 1 denies |
...
| P4 | Uniqueness, beyond IDF — not a theorem of logic; the mind conclusion does not need it (`CoreNoP4`). **Certified (v8.14):** IDF excludes position (ii) (`W_Two.Both`) and is silent on (i) and (iv) (`W_Two.OneChannel`, `W_P4`), where nothing is `Bare` | Position (ii): primitive thisness for necessary minds. Positions (i)/(iv): **nothing the model can name** — a second producer with no channel satisfies IDF, CE, and every field but P4/P4⁺ with no world bare; "Exit 1's bill for its share" needs a per-being notion of contribution the file lacks |
| IDF | That a fundamental being has no primitive thisness; and, certified, IDF alone is not P4 (`W_P4.IDF_and_not_P4`). IDF ≠ ID: different sorts; `W_P4` has IDF ∧ ¬ID (v8.14) | Primitive thisness for necessary beings, paid **separately** from CE's Objection 9 |
