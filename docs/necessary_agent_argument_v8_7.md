...
- (l) *(historical; P6′ retired v8.2, FA replaces it)* A second route to Tier 3 that does not use single-source derivation (D5): under P6′ — an ahistorical representing state is mental, derived or not — the agent itself is a mind (`T3_11'`), with no regress to a root. The argument now holds under *either* P6 + D5 or P6′; both are priced in §7.
...
**Changes from v8.16 (this revision — v8.17: the two formal burdens that every version since v8.10 listed as "the only remaining formal work", discharged).**

- (cx) **Per-being contribution, and CEB.** `Contrib w t` is the set of first items t causes at w; `ContribDet t` says it is world-invariant; `SensOf w t P` says the production is sensitive to P through a state *of t*; `BareOf w t` says t's contribution varies, no state of t is a channel at w, and no chance governs it; `Contributes t` says t contributes somewhere. **CEB** (`CEB_stmt`, optional): at every productive world, no fundamental being's contribution is bare — CE stated per being. Certified: `contrib_det_empty` (a world-invariant contribution is empty, because first items are contingent); `contributors_sensitive` and **`contributors_are_minds`** (under CEB and no chance, every fundamental that contributes to the first stage is field-sensitive through its own state at every productive world, and is a mind — CH, TR, FA); **`CEB_of_Axioms`** (CEB is a theorem of `Axioms`: for the one being, CE + NBL + CH + P4⁺). Witnesses: **`W_Two.OneChannel.position_i_priced`** — N₂'s contribution is bare at every world, N's is not, CEB fails; **`W_P4.alternation_priced`** — N₂ is bare at `true`, N at `false`, CEB fails; **`W_Two.Both.two_contributors_two_minds`** — CEB holds and both are minds. So the four positions of III.6 are now: (ii) satisfies CEB and its beings are minds; (i) and (iv) violate CEB, and their price is the premise the theist gets for free. Uniqueness is CEB + IDF + one further claim (profile uniformity), stated as such.
- (cy) **Fallible directedness, with content about the state.** `FallibleDir s D`: s is the channel at every productive world (`PowersChannel` — the causal role), D is realized at w₀, and D fails at some productive world. It mentions no field among `Sens`, `Rep`, `Mental`, and sits strictly between `manif` and `manif₀`. Certified: `fallible_not_power` (no necessitating power directs s at D — generalizes `no_powers_at_maximal`); `fallible_maximal_discriminates` (a fallible directedness at the best entails a productive world whose reality is not best: the residue's modal profile); `FallibleDir.toFallible` (the v8.9 structure is recovered); **`W_Pref.fallible_dir`** (the theist's state, with `Sens` at every world) and **`W_Nat.fallible_dir`** (the bare naturalist's, with no `Sens`) — the v8.9 claim "the same structure on both sides," now made with a definition that has content; **`NoTR.no_fallible_dir`, `Toy.no_fallible_dir`** (on a tied balance there is no fallible directedness at the best — Horn 1 admits only the necessitating reading). What separates the two sides is `Sens`, which is TR's question; that is where III.5's ceiling now sits, with nothing formal left on it.
- (cz) v8.17 was independently verified by CI run 94209860042 (all six checks; artifact byte-identical). The handoff's formal burden list is empty. What remains is philosophical: TR's content claim (III.5); IDF and profile uniformity (III.6); CatU's two premises (III.7); the problem of evil at P8/w₀.

**Changes from v8.15 (v8.16: the two items v8.15 deferred, and the two uncompiled conjectures).**
...
- (cm) **`mind_of_CE_NBL` concludes of the selector.** Its type ended in a fresh `∃ t', … ∧ M.Mind t'`, which the type left free to be another being; it now ends `M.Mental s ∧ M.Mind t` of the selecting state and its bearer. `DState.witness` now certifies that the ¬GG selector is itself a mind.
- (cn) **`T3_11'` proved directly.** It was `(T3_11 A).2`, whose proof goes through `root_of_state` and hence `A.Src`, contrary to three documents. The direct three-step proof does not mention `A.Src`; the type cannot certify that (`Axioms0` carries `Src`), and the prose now says "by inspection."
- (co) **L0 is a theorem of P1** (`L0_of_P1`: well-founded implies irreflexive). L0 stays a `CoreNoP4` field for continuity with the witness statements but is marked derived. This also withdraws v8.14's separate "L0 burden": a direction-neutral theory of causation (two-way links) is excluded by P1 itself.
- (cp) **Three uncertified claims certified.** `Toy.P8s_holds`, `Toy.SK_holds` (the optional premises are satisfiable with `Axioms`); `W_P4.IDF_and_not_ID` (IDF and ID are separate commitments — the file has a model of IDF ∧ ¬ID).
- (cq) **Definitions pinned in CI.** Check 5 (v8.14) pinned the printed *type* of each result, which names definitions without unfolding them; `def AccordsValue … := True` with patched proofs passed all five checks. Check 6 now pins the `#print` output of the `Model` structure and every definition, premise statement, premise structure, `God`, `KnowsAll`, and the powers structures against `expected_definitions.txt`. What green means after six checks is stated in `verify.sh` and README §0.0: the file declares these results with these axiom sets, types, and definitions — not that the definitions are the right ones, which is read, not run.
- (cr) **Prose corrections.** The IDF≠ID correction propagated to D25, the changelog, and §11; §11's TR and Src/D5 rows updated; the title no longer says "finished"; D23 marked; `Toy.Narrow` row corrected (it lacks ⟨cons c r⟩, not ⟨nil⟩); §5.4 "same content" is one direction; §10 item 15 no longer calls `ArchOrd` a dense ordered field (ℕ satisfies it: `Toy.archNat`); dead names (`identify_of`, `three_distinct`, P6′) removed or marked historical; D27 records that causally inert contingents fall outside `realOf`; D28 records that `Mind`/`Knows` do not require the state to be in force (tightening deferred); D18's "intensional" scoped to extension over all `Pred Item`; `no_modal_collapse`'s framing softened; Appendix A header corrected; README, guide body, and handoff synchronized.

**Changes from v8.13 (v8.14: after the third external review, which built its attacks as compiling countermodels; every one is now in the file).**

- (cc) **The fallible reading reopened; "one sentence wide" withdrawn.** `fallible_actual_iff` certifies that `FalliblePowers.actual M s` directs s at D iff the *actual* reality has D — the state does not occur in the definiens. So `W_Nat.fallible_witness` and `W_Pref.fallible_witness` certified only that the actual reality is maximal on each model's balance; both are deleted. Nothing with content about a fallible directedness has been formalized, so nothing has been shown about it (III.5). What remains certified is `no_powers_at_maximal`: no *necessitating* directedness at maximality lives on a discriminating balance. III.5's Mumford–Anjum, Vetter, and Bird paragraphs are rewritten to the texts: `manif` is Lowe's reconstruction of M&A, not their axiom; Vetter's degrees are a comparative without a measure (the file's own `AtLeast` is such an object), a third setting the v8.9 dial lacked; Bird allows stimulus-free probabilistic potencies, which route to III.7. The Oderberg step is shown to use "maximality" as a determinate type in the certified fork and as an abstraction in the philosophical step; it cannot be both.
- (cd) **`powers_fork`'s Horn 1 is the theist's residue.** `W_Pref.fork_is_residue`: a powers structure on the theist's own model, directed at "non-empty," makes the fork take its left horn at the world where `residue` holds — and the theist's `Sens`-channel "fails to track the good" there in the same sense. `natural_directed_nonempty`: on the structure `W_Nat.Pw` uses, every state of every model is "directed at a non-empty reality"; Horn 2's second conjunct is a tautology and `W_Nat.powers_witness` adds nothing to `W_Nat.witness`.
- (ce) **The two-fundamental positions certified; `W_P4` is (iv), not (ii).** `W_Two.Both.position_ii`: two fundamentals, both channels, both active at every world, `SameProfile`, IDF false — III.6's position (ii), which v8.10–8.13 wrongly identified with `W_P4`. `W_Two.OneChannel.position_i`: N₂ produces at every world with no channel state; IDF holds, CE holds, P4 fails, and **no world is `Bare`**. `Bare` is a predicate on worlds; "N₂ pays Exit 1's bill for its share" has no formal referent, and the same is true of alternation (`W_P4` is `Favored` at both worlds). Uniqueness is therefore certified to rest on IDF for position (ii) only; positions (i) and (iv) are **unpriced** in the model's terms until a per-being notion of contribution exists. IDF and ID are about different sorts (`Thing` vs first-stage `Item`) and are not "one commitment paid once."
- (cf) **The ¬NBL exit is a package.** `BothChannels.witness`: the law channel value-sensitive at every world, NBL false, every other field of `Axioms`, and `God` holds. `Axiarch` loses the conclusion because it also switches the state channel off. III.2's cost of denial and III.9's row now price the package `Axiarch`/`DLaw`, not the premise.
- (cg) **What `God` amounts to in the consistency witness.** `god_holds`: `God M .N` in `Toy.A`. `knows_everything`: that being accurately represents every set of items as an alternative *and as actual*, every reality including the empty one is maximal, and Ω is a singleton. `Accurate`, `ActsOnBest`, `AtLeast` are primitives with no axioms; `Maximal` need not be unique. §6.0 says so.
- (ch) **The uniform horn of the propensity fork certified.** `uniform_countable_refuted` (via `L2_countable_uniform`): P11 + **CatOpen** + L4 refute a uniform propensity over countably infinitely many realized categories. The v8.13 prose said "P11 and L4 alone"; CatOpen is needed, because L4 speaks only of open regions. The other two horns remain prose, and the middle horn is reclassified: `μ` and `Sens` are independent fields, so "a property-tracking chance is a selecting law" is a claim the model does not support.
- (ci) **CI pins statements.** A theorem whose statement was weakened to `… ∨ (1 = 1)` with the same proof term passed `verify.sh` byte-identically; the script now also diffs the `#check` output of every certified result against `expected_statements.txt`, and that attack is a tested failure.
- (cj) **Prose corrections.** III.1: the theist's residue is "which of the maximal class, and whether the inclination prevailed," not "one yes/no" (`Maximal` is not unique; in `Toy.A` every reality is maximal; the only discriminating certified balance, `W_Pref`, is defined by actuality); `T2_2` is `rfl` and cannot carry the transition/boundary-condition distinction, which is D3. III.4: the "count" is interpretive (the two packages differ formally in nothing but the `Q` in `Sens w Q (state s)`), and it asserts the belief–desire link the Smith reply denies; Collins 2019 engaged. III.6: L0 excludes direction-neutral causation, not P1. III.7: CatU has a second contestable premise (categorical similarity in species content; decoupling makes species-addition a limit); BHW 2018's replies reported; Schiffrin–Wald softened to the canonical measure. III.9 rows rewritten. Housekeeping: the README/handoff MD5 corrected; "prose only" commit labels noted as inaccurate.

**Changes from v8.12 (this revision — v8.13: the three remaining prose burdens; no Lean change).**

- (bz) **CatU given a positive argument** (III.7). The v8.5 appeal to parameter continua is withdrawn (a continuum is a manifold; manifolds are second-countable; on them kinds are points, not opens — objection (1) was right). What CatU and CatOpen jointly require is stated exactly: uncountably many *isolated* kinds. The positive argument delivers it from infinitely many independent *discrete* structural respects (field content, symmetry, dimension): a difference in any is a difference in kind and not a limit, and the subsets of a countably infinite menu of species are 2^ℵ₀. One contestable premise: that a law-structure with infinitely many independent features is possible. The countable-kinds skeptic is then met by the *propensity fork*: a uniform chance over countably many isolated kinds is refuted by P11 and L4 alone; a property-tracking chance is a selecting law (NBL); bare weights are III.4's count at the level of the measure. Prose, not theorem.
- (ca) **P1 argued against Schmid and Malpass** (III.6). The contradiction in the Grim Reaper story lives in the ungrounded chain of dependence — every finite truncation is consistent, and width does no work — so the unsatisfiable-pair diagnosis and causal finitism are not rivals: the second explains the first. The endless-future symmetry requires backward causation to be symmetric; without it the future story is not a causal chain, and with it P1 forbids the chain, not the future.
- (cb) **P11 argued against Benci–Horsten–Wenmackers** (III.7). NAP's ultrafilter-dependence is tolerable for credences and fatal for the objective propensity of a one-off (an ultrafilter-indexed family is not a chance, and an ultrafilter fixed by N's nature is a brute essential feature); NAP is not invariant under relabeling the kinds (Williamson); and infinitesimal propensity for every alternative explains the outcome no better than nothing.

**Changes from v8.11 (v8.12: IDF).**

- (bv) **IDF stated in the file.** `SameProfile t t'` (matched necessary states with the same content, mentality, accuracy, sensitivity) and `IDF_stmt` (fundamentals sharing every necessary feature are one) are defined as an optional premise. *(v8.12 said it was anti-haecceitism "made once" for P4 and CE's Objection 9; corrected in v8.14–8.15: they are separate commitments, D25.)*
- (bw) **Its limit certified.** `W_P4.IDF_and_not_P4`: the alternation model — two fundamentals, each active at a different world, with different necessary profiles — satisfies IDF vacuously and violates P4. IDF alone is not uniqueness; uniqueness is IDF plus CE's pricing of bare sources.
- (bx) **Horn (iv) added to III.6.** The v8.10 text described `W_P4` as the trilemma's position (i) or (ii); it is neither — it is *alternation*, and the contrast "which fundamental acts at w" is a bare contrast in its own currency. III.6 now has four positions and names its own witness correctly.
- (by) **IDF given its own dossier entry** (III.6): statement, what it is and is not (foundation-only, not global anti-haecceitism, not Black's spheres), the case, the certified limit, objections (Adams, hidden natures, symmetry), replies, cost, ceiling.

**Changes from v8.10 (v8.11: III.4 rewritten; no Lean change).**

- (bq) **Case (c) corrected.** The v8.5 text said the D-selector "accurately represents that D does not favor"; nothing in P5 or TR gives it that content. It now says what the model gives: complete accurate representation of every consideration (P5, P9), with the link from considerations to `AtLeast` stated as interpretive (D8, D16), and the cost of ¬GG counted as a second essential feature unconnected to the first.
- (br) **Smith answered on his own terms.** The distinct-existences thesis is metaphysical, not about finitude, and the model satisfies it: `Rep` and `Sens` are distinct primitives, neither entailing the other (`NoTR`, `NoMind`). GG is about the content of the essential `Sens`, on which Smith is silent.
- (bs) **The evil-god reply withdrawn and replaced.** "Evil = good + inversion" assumed GG. The certified core is symmetric under inverting `AtLeast`; the asymmetry lives in the interpretation of the balance as value-grounded reasons (D15, D16), and inverting it costs a theory of practical reason on which the bad is a reason for. Law's evidential parity relocates to the problem of evil (P8 at w₀), which the argument does not answer.
- (bt) **Dasgupta conceded fully.** "Only one is intelligible as agency" was GG restated; replaced by a count: the D-package posits two essential features and an unexplained relation between them, the good-package one feature whose aim is fixed by its representation.
- (bu) **The residue named in III.4.** GG gives direction; P7/P8 at w₀ give prevailing there; `W_Pref.residue` certifies it need not prevail elsewhere. The theist's agent at w′ does not exhibit a knowing preference for the worse; the GG-denier does not escape the residue and adds to it.

**Changes from v8.9 (v8.10: P4 narrowed to uniqueness).**

- (bn) **`CoreNoP4`.** `Core` is now `CoreNoP4` (every framework and Tier 1–2 premise except P4) plus P4, and `section CoreTheorems` is typed over `CoreNoP4`; only `T2_8` takes `Core`. So the types certify which results use uniqueness: `no_modal_collapse`, `T2_4`, `T2_15`, `mental_of_nonactual`, and `mind_of_CE_NBL` (with CE, NBL, CH, TR explicit) do not; `T2_8`, `identify`, `main`, `god_exists`, and the originator theorems do. All 49 prior results unchanged; verified by diff.
- (bo) **`W_P4.two_minds`.** In the ¬P4 model, N and N₂ are each fundamental at every world and each a mind. Denying P4 yields several necessary minds, not none: it is polytheism, not naturalism.
- (bp) **III.6 P4 rewritten.** The coordination argument developed as a trilemma for the second being — bare source (Exit 1's bill for its share), field-sensitive (a second mind), or mediated (P4⁺'s territory) — and uniqueness located at the premise it actually rests on: IDF, identity of indiscernible fundamentals, which is anti-haecceitism for fundamental beings *(v8.10 called it "the same commitment CE already needs against Objection 9"; withdrawn v8.14–8.15, D25)*. Island universes, the aggregate move, and the literature (Pruss & Rasmussen bracketing uniqueness; Rasmussen's pure actuality; Leftow's GSA; Gellman; Swinburne; Mghirbi's concession) placed. Open problem 6 is closed to the extent formal work can close it; what remains is IDF's defense, which is III.1's.

**Changes from v8.8 (v8.9: the fallible reading finished).**
...
- (s) **P4⁺ adopted.** "No necessary concrete being other than N" is now a premise (§2.2). Rationale: the argument could not decide whether the originator, the mind, and the agent are one being or several (§5.4), and of the premises that could decide it, P4⁺ is the one a skeptic has the least reason to deny — a skeptic who grants P4 has parsimony on the side of P4⁺, and the only standing motive to reject it is a doctrine of necessary generation, which is a theistic commitment. With P4⁺, `identify` is unconditional and `main` concludes about **one being**. The three-beings model (`W_Id.three_distinct`) becomes the independence witness for P4⁺. `identify_of` keeps the conditional form for readers who decline P4⁺. *(Historical: `W_Id.three_distinct` was replaced by `originator_neither` in v8.2; `identify_of` was removed when P4⁺ became a field.)*
...
- **D29** Per-being contribution (v8.17). `Contrib w t := fun x => F w x ∧ Causes w (inl t) x`; `BareOf w t := ¬ ContribDet t ∧ (∀ P, ¬ SensOf w t P) ∧ ¬ HasProp`. This is the notion III.6 needed and the file lacked from v8.10 to v8.16. Its consequences: a world-invariant contribution is empty (`contrib_det_empty`); positions (i) and (iv) are `¬CEB` (`position_i_priced`, `alternation_priced`); position (ii) is CEB with two minds (`two_contributors_two_minds`); CEB is derivable from `Axioms` (`CEB_of_Axioms`). What it does not do: yield uniqueness. Two channel-bearing contributors are excluded only if they share a profile (IDF), and profile uniformity is not a theorem — III.6 says so.
- **D30** Fallible directedness (v8.17). `FallibleDir s D := PowersChannel s ∧ D (realOf w₀) ∧ ∃ w, productive ∧ ¬ D (realOf w)`. The content about the state is the channel role; the failure clause separates it from a necessitating power (`fallible_not_power`) and from a tied balance (`no_fallible_dir`). Both the theist's state (`W_Pref`) and the bare naturalist's (`W_Nat`) satisfy it at maximality; they differ in `Sens`. D23's v8.9 sentences are now true of this definition, and the ceiling of III.5 is TR's content claim alone.
...
- **Contribution of t at w** (`Contrib w t`). The first items t causes at w. *(v8.17.)*
- **Contribution world-invariant** (`ContribDet t`). t's contribution is the same at every world. *(v8.17.)*
- **Sensitive through a state of t** (`SensOf w t P`). The production at w is sensitive to P through some state whose bearer is t. *(v8.17.)*
- **Bare contribution** (`BareOf w t`). t's contribution varies across worlds, no state of t is a channel at w, and no chance. *(v8.17; the per-being analogue of `Bare`.)*
- **Contributes** (`Contributes t`). t's contribution is non-empty at some world. *(v8.17.)*
...
| **P4** | There is an N such that, at every world, N is fundamental and every fundamental thing is N. | contested; used only for uniqueness and identification (v8.10: `CoreNoP4`); its denial yields several necessary minds (`W_P4.two_minds`), not none |
...
- **CEB** (`CEB_stmt`; v8.17). At every productive world, no fundamental being's contribution is bare. A theorem of `Axioms` for the one being (`CEB_of_Axioms`); as a premise in its own right it is what a second fundamental producer must satisfy (`W_Two.Both.CEB_holds`) or pay for (`W_Two.OneChannel.position_i_priced`, `W_P4.alternation_priced`). Under CEB and no chance, every fundamental that contributes is a mind (`contributors_are_minds`).
...
| **IDF** | Fundamental beings that share every necessary feature (`SameProfile`: matched necessary states with the same content, mentality, accuracy, and sensitivity) are identical. | nothing in the file; it is what P4's uniqueness rests on (III.6) | optional (v8.12); anti-haecceitism about the foundation; `W_P4.IDF_and_not_P4`: not sufficient for P4 on its own |
...
*Mind without value, corrected (v8.15).* `mind_of_CE_NBL` now concludes `M.Mental s ∧ M.Mind t` of the selecting state `s` and its bearer `t`. The v8.1–8.14 type ended in a fresh existential `∃ t', … ∧ M.Mind t'`, which the type left free to be a being other than the selector; the proof always instantiated `t' := t`, but the pinned object is the type.
...
- **3.11′** `T3_11'`. There is a necessary concrete Ag that is an agent at w₀, a mind, and knows all of Ω. *Proof:* the s of 3.6 represents the empty alternative (P5), is ahistorical and accurate (`selecting_props`); `mental_of_nonactual`. Unconditional since v8.2. *(v8.15: proved directly; the v8.2–8.14 proof was `(T3_11 A).2`, which goes through 3.7 and `Src`. The direct proof does not mention `A.Src`, by inspection — the type cannot say so, since `Axioms0` carries `Src`.)*
- **3.12 (v7)** `identify`. The N that is fundamental at every world is a mind, an agent at w₀, and knows all of Ω. *Proof:* both beings of 3.11 are necessary and concrete, hence by P4⁺ they are N.
...
- **Shared content.** The agent's selecting representation and the mind's root representation share content in one direction (Src, 3.7: the root represents everything the selector represents; nothing certifies the converse), and both are accurate (3.10). The two beings, if distinct, the root knows at least what the selector knows.
...
**What `God` amounts to in the consistency witness (v8.14).** `god_holds`: `God M .N` holds in `Toy.A`. `knows_everything`: the being there accurately represents every set of items as an alternative *and as the one that actually obtains* (`Accurate := True`), every reality including the empty one is maximal (`AtLeast := True`), and Ω is a singleton. Three of the conjuncts of `God` are as thin as their primitives: `Accurate` (L:874), `ActsOnBest` (L:901) and `AtLeast` (L:905) have no axioms, and `Maximal` need not be unique. The file has no model in which a balance specified independently of actuality discriminates: `W_Pref`'s balance is `r = actual ∨ r′ = ∅`. "Knows all of Ω" and "accords with the balance" are names for structural conjuncts, and the reader should weigh them as such (§8.4).
...
| NBL | `Toy.Axiarch` — value-sensitivity through the law channel, no state channel | the ground's law is sensitive to maximality as such (axiarchism) | 3.4: O agential (P10 fails) — **because the state channel is also off**; `Toy.BothChannels.witness` (v8.14): with it on, ¬NBL and `God` hold together |
...
| P5 | `Toy.Narrow` — Rep ⟨nil⟩ and every ⟨alt r⟩, no ⟨cons c r⟩ | s is selecting (TR holds) but represents no consideration bearing on any alternative | 3.6: a selecting rep represents all of Ω, considerations included |
...
| P4 | `W_P4` — N with s1 produces c1 at `true`; N2 with s2 produces c2 at `false` | two fundamental beings | 2.8: one being brings about every reality — but `W_P4.two_minds`: both beings are necessary minds. `W_P4` is III.6's position (iv), alternation; positions (i) and (ii) are `W_Two` (v8.14, rows below) | *(v8.15: `W_P2`, `W_P4`, `W_P1`, `W_L4` had listed `P6_stmt` in place of `FA_stmt` since v8.2; corrected.)*
...
| *(the TR fork, Horn 2)* | `W_Nat.powers_witness` — on `W_Nat`, s is a powers channel (`PowersChannel`) directed at "a non-empty reality" on the natural `Powers` structure, with no `Sens` through any channel | CE fails; nothing else | v8.14: `natural_directed_nonempty` shows the "directed at a non-empty reality" conjunct holds of every state of every model on this structure; what the witness adds over `W_Nat.witness` is nothing |
...
| *(Horn 1 is the residue)* | `W_Pref.fork_is_residue` (v8.14) — a powers structure on the theist's own model directed at "non-empty"; `powers_fork` takes its left horn at `false`, where `residue` holds; `Sens false Maximal (state s)` also holds there | nothing fails | "fails to track the good at some productive world" is a cost the theist certifiably pays too |
...
| *(per-being CE)* | `Toy.CEB_holds`, `W_Pref.CEB_holds` (v8.17, from `CEB_of_Axioms`) | nothing fails | CEB is a theorem of `Axioms` |
| *(position (i) priced)* | `W_Two.OneChannel.position_i_priced` (v8.17) | CEB fails: `BareOf w N₂` at every world, `¬ BareOf w N` | the bare source has a formal price |
| *(alternation priced)* | `W_P4.alternation_priced` (v8.17) | CEB fails: `BareOf true N₂`, `BareOf false N` | an idle turn is bare |
| *(two contributors, two minds)* | `W_Two.Both.two_contributors_two_minds` (v8.17) | P4 fails; CEB holds; both minds | `contributors_are_minds` instantiated |
| *(fallible directedness)* | `W_Pref.fallible_dir`, `W_Nat.fallible_dir` (v8.17) | nothing fails | the same contentful structure on both sides; they differ in `Sens` |
| *(no fallible directedness on a tied balance)* | `NoTR.no_fallible_dir`, `Toy.no_fallible_dir` (v8.17) | — | Horn 1 admits only the necessitating reading |
...
| 3 | 3D: reject D5 *and* FA *(P6′ retired v8.2)* | Multi-source derivation, and non-mental ahistorical representation of the non-actual (`NoMind`) | Partly | — |
...
| Single-source derivation (D5) | — | Used only for the root representation (3.7); the agent-is-a-mind result `T3_11'` does not need it (FA replaced P6′ in v8.2; direct proof v8.15) |
...
*Powers naturalist* (`NoTR`). Everything the bare naturalist grants, plus CE. What is left unexplained: how a state can be sensitive to a property of candidate realities without the candidates figuring — the fork in D23 (III.5). If they figure, the state ranges over them and TR follows; if they do not, `Sens` is false and this is `W_Nat`. *v8.14:* the fallible reading is open (III.5). A powers naturalist who holds a primitive tendency-modality that may fail with nothing interfering (Mumford–Anjum's own position, not Lowe's reconstruction of it) has exactly the modal profile of the theist's `Sens` (`W_Pref.residue`), and what he denies is TR's content claim and that alone. The dispute is one premise, TR; it is not "one sentence none of the powers accounts writes."
...
0. **TR — the fallible reading, formalized (v8.17)** (D23, D26, D30; III.5). Certified: `powers_fork`, `powers_all_tied`, `no_powers_at_maximal` (necessitating reading); `FallibleDir` with `fallible_not_power`, `fallible_maximal_discriminates`, `no_fallible_dir` on tied balances, and `W_Pref.fallible_dir` / `W_Nat.fallible_dir` (the same contentful structure on both sides, differing in `Sens`). Nothing formal remains. Open, and philosophical: TR's content claim — that a state whose contribution favors the best among the alternatives has them as content — against a powers naturalist who holds the theist's modal primitive and denies it.
1. **CatOpen and CatU** (P12 is now a theorem from them). CatOpen: supported empirically for the constants (D21). CatU: the v8.13 positive argument has *two* contestable premises (v8.14, III.7): that a law-structure with infinitely many independent features is possible, and that similarity is categorical in species content — under coupling-space similarity a species with vanishing coupling decouples and its addition *is* a limit. The countable-kinds skeptic meets the propensity fork, whose uniform horn is now certified (`uniform_countable_refuted`, from P11 + **CatOpen** + L4) and whose other two horns are prose; the middle horn ("property-tracking is a selecting law") is a reclassification `μ`/`Sens` independence does not support.
...
6. **P4 — finished formally (v8.17).** Per-being contribution (D29) and CEB. `contributors_are_minds`: under CEB and no chance every fundamental contributor is a mind. (i) and (iv) violate CEB (`position_i_priced`, `alternation_priced`); (ii) satisfies it and is IDF's job. CEB is a theorem of `Axioms` (`CEB_of_Axioms`). What remains, philosophical: IDF against a foundation-haecceitist, and profile uniformity — that two necessary contributors' channel states match in content, mentality, accuracy and sensitivity, which P5/P9/GG do not entail.
...
15. **Closed.** No external library is needed. `ArchOrd` is instantiated by ℚ in core Lean (`archRat`) and by ℕ (`Toy.archNat`); it is a weak axiomatization — a reflexive-transitive `le` (no antisymmetry or totality), a commutative associative `add` with zero, monotonicity, and two Archimedean clauses; no density, negation, or multiplication. That ℝ satisfies it is verified by inspection. *(v8.15: the earlier "certified to be those of an ordinary dense ordered field" was false — ℕ satisfies them.)* The trusted base is core Lean only.
16. **Faithfulness review.** An independent reading of Part I against the Lean file by someone who did not write both.
17. **IDF** (v8.12; scoped v8.14; positioned v8.17). Stated as an optional premise (III.6). Certified: not sufficient for P4 alone (`W_P4.IDF_and_not_P4`); sufficient against position (ii) (`W_Two.Both.not_IDF`); separate from ID in both directions (`W_P4.IDF_and_not_ID`, `W_Two.Both.ID_and_not_IDF`). After CEB (v8.17) its job is (ii) alone. Remaining: its defense, and profile uniformity.
...
| P1 | Contested; well-foundedness only (v8.4); the Grim Reaper diagnosis argued (v8.13): the contradiction lives in the ungrounded chain (finite truncations are consistent), not in width or bare inconsistency; the endless-future symmetry needs backward causation to be symmetric | An answer for a theorist who denies causation has a direction |
...
| GG | Contested (guise of the good); certified independent (`DState`); denial keeps the mind; Smith's distinct-existences thesis satisfied by the `Rep`/`Sens` split (v8.11); the evil-god symmetry located at the interpretation of `AtLeast` and priced as a theory of reasons | The Velleman/Stocker debate; whether badness can ground reasons for |
| P3 | Derived from CE + NBL + GG | — |
| P4 | Used only for uniqueness and identification (`CoreNoP4`); the mind conclusion survives its denial. Per-being contribution (v8.17): under CEB every fundamental contributor is a mind (`contributors_are_minds`); (i)/(iv) violate CEB and are priced (`BareOf`); (ii) is excluded by IDF given profile uniformity. CEB is a theorem of `Axioms` | IDF's defense; profile uniformity |
...
| TR | **The logical load of the argument.** Certified independent by a mindless `NoTR`. Necessitating reading closed (v8.8). Fallible reading formalized with content about the state (v8.17, `FallibleDir`): not a power, needs a discriminating balance, absent on tied balances, satisfied by the theist's and the bare naturalist's states alike, which differ in `Sens`. Nothing formal remains | TR's content claim: the case in III.5 (a)–(c) and the Oderberg step |
| P10 | Derived from NI + CH + TR | — |
| P11 | Contested; stated as Archimedean values + finitely additive monotone measure; the standard view; against NAP the dossier now argues (v8.13) that an ultrafilter-indexed family is not a propensity of a one-off, that NAP is not invariant under relabeling kinds, and that infinitesimal chance for every alternative explains nothing | Resolution of the regularity debate; an NAP account of objective chance, which the literature does not offer |
...
| CatU (optional) | Contested; positive argument from infinitely many independent discrete respects (v8.13); one contestable premise (a law-structure with infinitely many independent features is possible); the countable case caught by the propensity fork (prose) | The modal possibility of infinite independent structure; a certified propensity fork |
...
| Src / D5 | Stipulation; unargued; used only to reach the root representation (3.7, `T3_11`); the agent-is-a-mind result `T3_11'` has a direct proof that does not mention it (v8.15, by inspection) | An account of derivation among necessary states |
...
| IDF (optional, v8.12) | Anti-haecceitism about the foundation; what P4's uniqueness and CE's reply to the haecceitistic exit both rest on; certified not sufficient for P4 alone (`W_P4.IDF_and_not_P4`) | A defense of restricted anti-haecceitism against Adams-style thisness at the foundation |
...
**To 5 (modal collapse).** Closed by theorem. `no_modal_collapse` proves that the premises, CE included, leave the first-stage output contingent: some two worlds have different outputs. *Scoped (v8.15):* this is near-analytic — any contingent item fails to exist at some world, so `F` varies (that is the whole of `T2_4`); it does not test the premises' modal strength, and the Gödel/Scott comparison in the docstring is a comparison of *shapes*, not a result. CE does not necessitate anything; it says something mattered, not that it settled the matter. The collapse objection targets principles above CE on the ladder.
...
*Corrected (v8.14).* "One yes/no" is the best case, not the certified one. `Maximal` is not unique, `exists_maximal` gives a maximal element and not a unique one, and `Sens w Maximal` discriminates nothing *within* the maximal class; P8 is silent on which member obtains. In the consistency witness `Toy.A` every reality is maximal (`knows_everything`), so there the theist's package explains nothing about content; the only certified model of `Axioms` with a discriminating balance is `W_Pref`, whose balance is *defined* by naming the actual reality. The first asymmetry therefore reads: the theist explains the outcome *up to the maximal class*, whose size the argument cannot bound, and leaves "which of the tied, and whether the inclination prevailed" unexplained; the bare naturalist leaves the whole content. Whether the first is smaller than the second is a comparison the file does not certify and the prose should not round.
...
```
∀ w s, M.E_state w s → ¬ M.Mental s → ¬ M.Historical w s →
  (∀ r, M.Rep s (.alt r) → r = M.realOf w) ∧ (M.Rep s .nil → ∀ x, ¬ M.realOf w x)
```

*Corrected (v8.15).* The hypothesis `M.E_state w s` — the state is in force at `w` — is new. Without it, `¬ M.Historical w s` holds vacuously at every world where `s` is absent (F1: nothing causes an absent state), and since `Rep` is not world-indexed the v8.2–8.14 FA required every non-mental state that represents an alternative `r` to have `r = realOf w` at *every* world where it is absent: a contingent organism's non-mental detector state, absent at two worlds with different realities, violated it. The formal FA was much stronger than this dossier's, and was denied by exactly the biological representation D19 says it leaves untouched. The corrected FA is the premise defended below. Nothing certified changes: every use in the proofs applies FA to a state that has caused a first item at `w`, hence is in force there. P9 received the same restriction for the same reason (a necessary state of a *contingent* being is vacuously ahistorical wherever its bearer is absent).

In plain terms: a mindless state with no history, in force at a world, can carry content only about what is the case at that world. It cannot represent what is not the case, what might have been, or the absence of everything. FA replaces P6. It is not a claim about minds; it is a claim about the *limits of non-mental content*, and the mind conclusion follows from it by elimination, because the selecting state represents producing nothing as an alternative (TR, P5) and something exists.
...
**(c) Selection for a non-evaluative property is a second essential feature, unconnected to the first.** *Rewritten in v8.11; the v8.5 version claimed the D-selector "accurately represents that D does not favor," which nothing in P5 or TR gives it.* What the model gives the selector is this: by P5 its state represents every alternative and every consideration bearing on each (`cons c r` for every c with `Bears c r`), and by P9 it is accurate throughout. On the intended reading (D8, D16) the balance `AtLeast` is what those considerations jointly support — the Lean does not tie `Bears` to `AtLeast`, and this link is interpretive, stated as such. Now suppose the selector is essentially directed at a descriptive D — "produce a reality with at least one particle" — rather than at maximality. Then the being has two essential features: a complete and accurate representation of every consideration bearing on every alternative, and an aim whose content is independent of that representation. In the theist's package there is one such feature: the aim is *at what the representation shows to be favored* — its content is fixed by the representation, not added to it. In the D-package the aim ignores what the representation contains, and nothing connects them. That is not an appeal to "intelligibility"; it is a count. The D-selector posits an unexplained relation — an essential aim, in a complete knower, unrelated to what it knows — that the good-selector does not. GG is the claim that the foundation's selector has the one-feature structure.

*Scoped (v8.14).* The count is interpretive and the reader should know exactly how. Formally `Toy.A` and `Toy.DState` differ in nothing but the `Q` in `Sens w Q (state s)` (`viaState` vs `Q = D`); each has one representation primitive (`Rep := True` in both) and one sensitivity primitive; nothing in `Model` connects `Rep` content to `Sens`, and `Rep s (.alt r)` holds for every `r` in both, so the D-selector's aim is at a property of realities its representation represents, exactly as the good-selector's is. "The aim's content is fixed by the representation" has no formal counterpart for *either* package; it is a premise about a link the file does not have, and it is the belief–desire link the reply to Objection 2 says the model does *not* assert. One of the two replies must be given up; the dossier keeps the count as an interpretive premise and withdraws the claim that it is "not an appeal to intelligibility."
...
**To 2 (Humean motivation).** *Rewritten in v8.11.* The v8.5 reply said Smith's thesis "concerns the contingent psychology of finite agents." The review was right that it does not: Smith's argument is that belief and desire are distinct existences with opposite directions of fit, and that is a claim about the metaphysics of motivation, not about finitude. It applies to necessary states as much as to contingent ones. The reply is that the model *satisfies* it. The argument does not derive motivation from representation. It has two primitives: `Rep` — the state's content (belief-like; TR, FA, P5, P9) — and `Sens` — the production's sensitivity through that state (desire-like; D20: inclination, not necessitation). They are distinct in the model, neither entails the other, and `Toy.NoTR` and `Toy.NoMind` certify that each can hold without the other. So no premise says that knowing the best entails being moved toward it. GG is a claim about the *content* of the essential `Sens` — that in a being with complete accurate representation and no contingent motivation, the inclination is toward maximality — and Smith's thesis is silent on that. The Humean who presses on is no longer applying Smith; he is asserting that a necessary being's essential inclination may be unrelated to what it essentially and accurately represents. That is possible — `Toy.DState` is its model, and it keeps the mind — and its cost is (c): a second essential feature unconnected to the first.

**To 3 (evil god).** *Rewritten in v8.11.* The v8.5 reply said the evil god is "the good one plus a brute essential inversion." The review was right that this holds only under the guise of the good, which is the contested premise; an evil-theorist says the reverse. So the reply is withdrawn and replaced by a statement of where the asymmetry lives. The certified core is *symmetric* under inverting the balance: `AtLeast` is a primitive comparative with no axioms (D16), and if it is read as "at least as bad as," every theorem still holds and `god_exists` delivers a necessary mind that selected the worst reality. The argument never claimed otherwise: that the balance is a *value* ordering, and that its reasons include moral ones, are interpretive premises (D15). Law's challenge is therefore aimed at exactly one thing — the interpretation of `AtLeast` — and what it costs to invert it is this. The balance is the balance of *value-grounded reasons*: considerations that count in favor of a reality count in its favor because of what is good about it. The evil reading requires that badness ground reasons *for*: that a reality's being worse is a consideration in favor of producing it. That is not a claim about God; it is a theory of practical reason — that the bad favors — and it is one no one holds, Law included: his challenge is an *evidential* parity argument (the world's mixed character supports evil-god as well as good-god), not a claim that badness is reason-giving. Against an evidential argument for goodness, Law's parity is fatal; against this argument, which concludes only that the outcome accords with the balance of reasons, it relocates to the theory of reasons, where the asymmetry between good and bad is the tradition's (Raz 2010: reasons are given by value). The evil-god theorist may deny that theory. Its price is a practical reason on which the bad is a reason for, and that is the price, stated without assuming GG. What Law's evidential point does touch is P8 at w₀ — whether the *actual* reality accords with the value balance — and that is the problem of evil, which the argument locates and does not answer (§8.4).

*Collins (v8.14).* Collins (2019) is the paper that extends Law to arguments of this shape, and its point is that every asymmetry-breaking move — including the appeal to what grounds reasons — has a mirror the evil-god theorist adopts: reasons grounded in disvalue. "No one holds it" is a headcount, not a reply, and the certified core's symmetry under inverting `AtLeast` is what makes the headcount the only thing left. Raz 2010 argues *for* the guise of the good; citing him for the asymmetry cites the thesis at issue. The honest ceiling is: the argument concludes accordance with a primitive comparative; that the comparative is value is D15/D16, interpretive; the evil-god theorist reads it as disvalue at no formal cost, and the price named here is a price only if reasons are value-grounded, which is GG.
...
**To 5 (essential aims are autonomous).** *Rewritten in v8.11.* The v8.5 reply conceded "partway" and then said that of the two essential directednesses "only one is intelligible as agency," which the review correctly identified as GG restated. The concession should be full and the reply different. Dasgupta's exemption is granted: an essential directedness at D, as a fact about what the being is, is autonomous and does not *demand* explanation. Autonomy exempts a fact from the demand; it does not exempt a package from the count. The D-package posits two essential features — a complete accurate representation of every consideration, and an aim whose content is independent of it — and the relation between them, that the aim ignores the representation, is a third fact about the being that is neither of the two and is explained by neither. The good-package posits one essential feature with two aspects: a representation, and an aim at what the representation shows to be favored, whose content is fixed by the representation. Both packages are autonomous at every essential fact they contain; one contains an unexplained relation the other does not. That is a difference in what is posited, not in what is intelligible, and Dasgupta's exemption does not close it. GG is the claim that the foundation's selector is the one-feature kind, and its ceiling is that a skeptic can posit the two-feature kind and pay the count.

**The residue, named.** The review's Attack 1 reaches this section too, and it is conceded here as in III.1. GG gives the *direction* of the essential inclination: toward maximality. P7 and P8, at w₀ only, give that the inclination prevailed there. Nothing gives that it prevails elsewhere, and `W_Pref.residue` certifies that it need not: at w′ the same agent, same state, same complete accurate representation, produces the worse. So "a knower that acts on what it knows" is delivered as a fact about w₀, not as a nature. That is D20 — `Sens` is inclination — and it is priced in D22. What should not be said is that the theist's agent at w′ exhibits "a knowing essential preference against reason": it does not. Its essential inclination is toward the best; the inclination did not prevail; nothing explains why. That is one binary residue. The GG-denier's agent has a different profile: an essential aim at D, which either necessitates D at every world (then the contrast among D-realities is bare, and the good is tracked nowhere) or is itself an inclination that may fail (then the residue is there too, plus the second essential feature). The GG-denier does not escape the residue; he adds to it.

### Cost of denial

To deny GG: a necessary mind with complete accurate representation of every alternative and every consideration, and no contingent motivation, has an essential inclination whose content is independent of what it represents — a second essential feature, unconnected to the first, plus the unexplained relation between them. `Toy.DState` certifies consistency and certifies that the mind survives. To deny P8(a): the inclination is essentially toward what the balance rates lower — which, if the balance is the value ordering, is a practical reason on which the bad is a reason for, and if the balance is inverted, is the same argument with the words exchanged. To deny P8(b) without P8s: a best reality does not exist, and the premises are jointly false; with P8s: the actual reality is good rather than unbeaten. What every denier shares with the theist: the residue.

### Ceiling

GG and P8(a) cannot be proved against a Humean who asserts a necessary being's essential inclination may be unrelated to its essential knowledge, or against an evil-god theorist who holds that badness grounds reasons for. They can be shown to satisfy Smith's distinct-existences thesis rather than violate it; to cost the denier a second essential feature and an unexplained relation between it and the first, whatever Dasgupta's exemption says about explanation; to be symmetric under inverting the balance at the formal level, with the asymmetry located in the theory of reasons and priced there; and to deliver accordance at w₀ only, with the residue at w′ conceded and shared. P8(b) commits to a best reality, and P8s is certified as the alternative. The skeptic who holds firm keeps the necessary mind and pays with a knower whose essential inclination has nothing to do with what it knows.

### Independent literature

Anscombe, *Intention* (Blackwell, 1957). Raz, "On the Guise of the Good," in Tenenbaum (ed.), *Desire, Practical Reason, and the Good* (Oxford, 2010). Tenenbaum, *Appearances of the Good* (Cambridge, 2007). Velleman, "The Guise of the Good," *Noûs* 26 (1992). Stocker, "Desiring the Bad," *Journal of Philosophy* 76 (1979). Setiya, *Reasons without Rationalism* (Princeton, 2007). Smith, *The Moral Problem* (Blackwell, 1994). Law, "The Evil-God Challenge," *Religious Studies* 46 (2010). Rowe, *Can God Be Free?* (Oxford, 2004). Kraay, "Theism, Possible Worlds, and the Multiverse," *Philosophical Studies* 147 (2010). Slote, "Satisficing Consequentialism," *PAS Supp.* 58 (1984). Tucker, "Satisficing and Motivated Submaximization," *PPR* 93 (2016). Dasgupta, "Metaphysical Rationalism," *Noûs* 50 (2016). Hume, *Dialogues Concerning Natural Religion* (1779), X–XI. Collins, "The Evil-God Challenge: Extended and Defended," *Religious Studies* 55 (2019). Forrest, "Replying to the Anti-God Challenge," *Religious Studies* 48 (2012).
...
*Horn 2 — a determinate type.* Then the state is directed at D the way a magnet is directed at north: the production manifests D and no alternative figures. `W_Nat.powers_witness` certifies what this is: on the bare naturalist's own model, s is a powers channel directed at "a non-empty reality" on the natural `Powers` structure, with no `Sens` through any channel, and CE false. A power directed at a determinate type, with the alternatives not figuring, *is* `W_Nat` — Exit 1, with Exit 1's bill. And `powers_fork` says, for any D that is not maximality on some productive world, that the channel does not track the good there: GG fails for it, the powers naturalist is denying two premises, not one, and the contrast among the D-realities — which non-empty reality? — is a contrast to which nothing bore. *v8.14:* two corrections. `natural_directed_nonempty` certifies that on the structure `W_Nat.Pw` uses, *every* state of *every* model is "directed at a non-empty reality" (`F w ⊆ realOf w`); so "directed at a determinate type" is a tautology of the structure and `powers_witness` adds nothing to `W_Nat.witness`. And `W_Pref.fork_is_residue` certifies that on the *theist's* model a powers structure directed at "non-empty" makes `powers_fork` take this same horn at w′, where `residue` holds and the theist's `Sens`-channel "fails to track the good" in the same sense. Horn 1's cost is one the theist certifiably pays; the fork prices nothing beyond `W_Pref.residue`.
...
*The fallible reading (v8.9; reopened v8.14).* The powers naturalist's remaining position is a directedness at maximality that, unlike a power, may fail at w′. `W_Pref.no_powers_at_maximal` shows it is not a `Powers` structure — that is, not a *necessitating* directedness. v8.9 then claimed to show it is "the same structure on both sides" via `FalliblePowers.actual` and two witnesses. That claim is withdrawn. `fallible_actual_iff` certifies that `(FalliblePowers.actual M s).Directed s D ↔ D (M.realOf M.w₀)`: the state `s` does not occur on the right; every state of every model is "fallibly directed" at every property the actual world has (`fallible_actual_trivial`: at `fun _ => True` and at "being the actual reality"). The two witnesses certified one thing each — that the actual reality is maximal on that model's balance — and are deleted. What a fallible directedness *is* (a structure weaker than `manif`, stronger than `manif₀`, with content about `s`) has not been formalized, so nothing has been shown about it. What follows is the prose on the powers theorists' own accounts, corrected to their texts.

*Mumford and Anjum.* *Getting Causes from Powers* (2011) argues against necessitation on two grounds. The first (ch. 3, the antecedent-strengthening argument) is interference: no set of powers necessitates because something further could be added. The second is that the dispositional modality is *sui generis* — weaker than necessity *even for the total cause with nothing further to add* — and they explicitly refuse the reconstruction "necessitates in the absence of interferers," because it treats the tendency as conditional necessity. Lowe (2012) is precisely the objection that their argument establishes only conditional necessity; Bird (2016) presses the obscurity of the "less than necessity" modality. The v8.9 text adopted the critics' reconstruction and attributed `manif` to Mumford and Anjum as "the powers theorist's own axiom." It is Lowe's axiom for what they *ought* to say; it is not theirs. On their own account a tendency at the foundation may fail with nothing interfering, and the v8.9 charge — that a primitive modality "supplies no reason the failure world differs from the success world, which is what bare means" — is exactly the charge the argument concedes against its own `Sens`: `W_Pref.residue` is a primitive inclination that supplies no reason w′ differs from w₀. If that is what "bare" means, the theist's package is bare at w′ by the same criterion. The powers naturalist who takes Mumford and Anjum's primitive has the theist's modal profile, and what separates them is TR's content claim, on which *Getting Causes from Powers* is silent.

*Vetter.* On *Potentiality* (2015) a potentiality is individuated by its manifestation, has no stimulus, need not manifest, and its non-manifestation is primitive: no rival and no trigger is owed (ch. 3, against the conditional analysis). Degrees (§3.4–3.5) are a *comparative* — x has more of a potentiality to M than y — tied to a comparative possibility, and Vetter distinguishes them from probabilities. The v8.9 fork ("its degree — then a propensity structure, `HasProp`; or nothing — then bare; no third setting") is not hers, and it is not exhaustive: a comparative without a measure is neither `HasProp` (a distribution with `μ`, D11) nor nothing. The file's own balance `AtLeast` (L:905) is exactly such an object — a comparative with no axioms and no measure. So the dial has the setting the argument uses for itself. A powers naturalist can hold Vetter-degrees over N's potentialities at the foundation and meet neither III.7 nor "bare," and the argument owes him a reason why a comparative among N's potentialities is a different kind of object from the comparative among realities the theist calls the balance. It has not given one.

*Bird.* *Nature's Metaphysics* (2007) treats spontaneous manifestations — radioactive decay is the standing example — as potencies with a trivial or null stimulus condition and a probabilistic manifestation (ch. 2 on propensities; ch. 3 on fundamental potencies). A potency that manifests at one world and not another with nothing distinguishing them is a *probabilistic* potency, which is III.7's territory, not a non-Birdian object. The v8.9 dichotomy (no stimulus, therefore not Birdian) is not in the text; Bird routes the powers naturalist to the anti-chance tier, whose ceiling is now CatU's two contestable premises and the propensity fork (III.7).

*Oderberg, and "maximality."* Horn 1 of the certified fork treats `Directed s M.Maximal` as a determinate-type power: "a power directed at the best manifests the best at every world." The philosophical step then says a state directed at *whichever* is best exhibits the specific indifference Oderberg (2017) reserves for the mental. These are two readings of "maximality." `M.Maximal : Reality → Prop` is a property of realities; a power directed at it is directed at a type exactly as a power directed at "non-empty" is, and Oderberg's abstraction — directedness at a universal abstracted from particulars, as against a power's fixed directedness at *its* manifestation type — gives no reason such a state is mental ("dissolving" is "whichever dissolution" too). Either maximality is a determinate type, and Horn 1 stands but Oderberg does no work; or it is not, and the powers naturalist never offered it and Horn 1 is empty. The Oderberg step applies only to a state that represents the *comparison*, which is TR assumed.

*What is left.* A state that inclines toward the best among the candidate realities and may not prevail. That is `Sens` in D20's sense, and it is the theist's state (`W_Pref.residue`). The powers naturalist who holds it as a primitive modality — Mumford and Anjum's, or Vetter's comparative — holds the theist's modal profile and denies that the state has the alternatives as content. That denial is ¬TR, and it is one premise; it is not "one sentence none of the powers accounts writes," because the powers accounts do not write TR either way.

*The fallible reading, formalized (v8.17).* `FallibleDir s D` (D30): s is the channel at every productive world, D holds at w₀, D fails at some productive world. This is what a fallible directedness *is* with content about the state — the channel role — and it is strictly between `manif` and `manif₀`. Certified: no necessitating power directs s at D (`fallible_not_power`); a fallible directedness at maximality forces a discriminating balance and a productive world where the outcome is not best (`fallible_maximal_discriminates`) — the modal profile of `W_Pref.residue`; on a tied balance there is no such state (`NoTR.no_fallible_dir`, `Toy.no_fallible_dir`), so Horn 1 admits only the necessitating reading; and the theist's selecting state (`W_Pref.fallible_dir`, with `Sens` at every world) and the bare naturalist's channel state (`W_Nat.fallible_dir`, with no `Sens` anywhere) both satisfy it. So the v8.9 sentence "the same structure on both sides" is now true of a definition that has content, and what distinguishes the sides is exactly `Sens` — whether the alternatives figure. Nothing formal is left on the fallible reading; what is left is TR's content claim, argued in (a)–(c) above and at the Oderberg step, and it is philosophical.
...
To deny TR is to hold that a state of the necessary ground through which the first production is sensitive to a property of the candidate realities, and in virtue of which the ground produces, has no alternative as content — including the alternative of producing nothing. `Toy.NoTR` certifies the position is consistent and mindless. What is certified about its price (v8.14): a *necessitating* directedness (`manif`) at a determinate type is `W_Nat` under another name (`W_Nat.powers_witness`, though its "determinate type" conjunct is a tautology), and at maximality ties every possible reality (`powers_all_tied`) or contradicts a discriminating balance (`no_powers_at_maximal`). What is *not* certified: anything about a directedness weaker than `manif`. `powers_fork`'s "fails to track the good" horn is `W_Pref.residue` on the theist's own model (`W_Pref.fork_is_residue`). The fallible reading is not a tendency, potentiality, or disposition *on the v8.9 readings of those accounts*, and those readings do not hold to the texts (above). So denying TR costs exactly this: holding the theist's modal primitive — an inclination toward the best that may lose, with nothing distinguishing the worlds — while denying that it is about any alternative. That is one premise, TR, and the argument's case for it is (a)–(c) above plus the Oderberg step, which is philosophical and applies only under the reading Horn 1 denies.

### Ceiling

TR cannot be proved against a theorist who asserts, as a primitive, a fallible inclination toward the best among alternatives that is about none of them. What can be shown, and is: that a *necessitating* powers channel ties every reality or fails on a discriminating balance (`powers_fork`, `powers_all_tied`, `no_powers_at_maximal`); and (v8.17) that a *fallible* directedness at the best, defined with content about the state (`FallibleDir`), is not a power, requires a discriminating balance and a world where it loses, and is satisfied by the theist's state and the bare naturalist's alike (`W_Pref.fallible_dir`, `W_Nat.fallible_dir`), which differ in `Sens`. What cannot be shown, and was asserted in v8.9–8.13: that a fallible directedness is "not a tendency, potentiality, or disposition" (the texts do not say so); or that its holder owes one sentence — he owes the same modal primitive the theist holds, and the dispute is entirely TR's content claim, on which the powers literature is silent. The powers naturalist's exit is therefore TR, whole; its price is the case for TR in (a)–(c), the Oderberg step under the field-ranging reading, and nothing certified.
...
**What changed in v8.4.** P1 previously said every item has finitely many ancestors, which is stronger than the proofs need and false of any continuum physics taken at face value (an event's past light cone contains infinitely many events). The proofs use P1 only to find *minimal* elements (`exists_minimal`, by well-founded induction), and well-foundedness suffices for that. So P1 permits infinite branching — an item with infinitely many causes — and forbids only an infinite regress.

**Case, rewritten (v8.13).** The Grim Reaper family (Benardete 1964; Koons 2014; Pruss 2018) is the case, and it should be stated in the form P1 actually needs. Reaper *n* is set to act at time 1/*n* iff no reaper *m* > *n* has already acted. Each reaper's action depends on the actions of all higher-numbered reapers: in the order of dependence, reaper 1 depends on reaper 2, which depends on reaper 3, and so on without end. That is an infinite *descending chain* of dependence — a failure of well-foundedness — and it is exactly and only what P1 forbids. The contradiction (some reaper must act, since otherwise reaper 1 would; but no reaper can, since a higher-numbered one would have) arises from the chain and from nothing else: every finite truncation of the story — reapers 1 to *N* — is consistent, because reaper *N* acts and the rest stand down. Width does no work. Infinitely many reapers acting *independently* of one another — infinite branching — generate no contradiction, because no reaper's instruction then refers to another's. That is why P1 forbids descent and permits branching: the paradox lives in the chain, and continuum physics lives in the branching.

**Objections, at full strength.** (1) *The unsatisfiable-pair diagnosis* (Shackel; Schmid 2024, "Benardete Paradoxes, Patchwork Principles, and the Infinite Past"): the reaper story is contradictory because its conditional instructions are jointly unsatisfiable, and an unsatisfiable set of conditions is impossible whatever the metaphysics of causation. No moral about infinite chains follows from the impossibility of an inconsistent story, and the patchwork principles the finitist uses to argue that the story *would* be possible if infinite chains were overgenerate elsewhere. (2) *The endless-future symmetry* (Schmid, "The End Is Near," *Mind*): a mirror-image reaper story runs toward the future — reaper *n* at time *n* acts iff no reaper *m* > *n* will act — and yields the same contradiction; if the paradox rules out an infinite past it rules out an endless future, which no one accepts; so the paradox proves too much and supports no conclusion about the past. (3) *Branching actualism* (Schmid & Malpass 2023): the reaper scenarios are not genuinely possible in the way the argument needs. (4) *Infinitism* (Oppy; Meyer): an infinite regress of contingent causes is simply what reality is like.

**Replies.** (1) The unsatisfiable-pair diagnosis and causal finitism are not competing explanations of the contradiction; the second explains the first. The reapers' instructions are individually consistent, and any finite set of them is jointly consistent. They become jointly unsatisfiable *only* when the set is infinite and each depends on all its successors. So "the conditions are unsatisfiable" is true, and the question is why infinitely many individually satisfiable, pairwise compatible, finitely-jointly-satisfiable conditions become unsatisfiable together. The answer is that the chain of dependence has no minimal element: there is no reaper whose action is settled first, from which the rest could be settled. That is a failure of well-foundedness, and it is what the contradiction turns on. The finitist's moral is then not "this story is impossible" but "what makes this story impossible — an ungrounded chain of dependence — is impossible wherever it occurs," because wherever it occurs the same instructions can be attached. Schmid's attack on patchwork principles is well taken as an attack on the *possibility* argument (that the story would be possible absent finitism); P1 does not need that argument. It needs only that the source of the contradiction is the chain, which the finite truncations show directly. (2) The future-directed story requires each reaper's action to depend on the actions of *later* reapers. Without backward causation, later actions are not among the causes of earlier ones, and the dependence in the story is not causal dependence at all; P1, which is about causal ancestry, says nothing about it, and the story is then a set of conditions with no causal reading — an unsatisfiable pair in Schmid's own sense, and impossible for that reason. With backward causation admitted, the future story *is* an infinite descending causal chain, and P1 forbids it — which rules out an endless future with infinite backward dependence, not an endless future. So the asymmetry between the two stories is the asymmetry of causation, not a line drawn to protect the past, and P1's defender can grant the future story its impossibility on either reading. The dossier does not claim this settles the literature; it claims the symmetry objection needs backward causation to be symmetric, and that with backward causation P1 is not embarrassed. (3) Branching actualism, as §7 Exit 2E notes, does not escape the argument; it instantiates the ground as a necessary initial state and relocates the dispute to CE or TR. As a reply to the reaper argument specifically, it costs the branching actualist an independent account of why the scenario is impossible — which, if it is not finitism, is the unsatisfiable-pair diagnosis, answered in (1). (4) Infinitism is coherent, and `W_P1` is its model. Its cost is the paradoxes, plus a second point: an infinite chain of contingent items is itself a contingent totality, and the infinitist owes an account of it that is not Hume–Edwards (Rowe). The infinitist who says "the chain has no explanation and needs none" is at Exit 1B for the chain.

**Cost of denial.** An ungrounded chain of dependence, and with it the reaper contradiction, or an account of why the contradiction arises that does not mention the chain — which the finite truncations show cannot be given. **Ceiling.** P1 rests on the diagnosis that the chain, not the width and not the mere inconsistency, is what the paradox exposes. The unsatisfiable-pair theorist can insist that the story's impossibility is brute; the reply is that its finite fragments are all possible, and that "impossible only when infinite and ungrounded" is a fact about ungrounded infinity. The future-symmetry objection is answered by the direction of causation; a theorist who denies that causation has a direction owes P1 a different reply, and the dossier has not given one for that case. *v8.14–8.15, on the direction-neutral theorist:* with `Causes` symmetric (Price; a Humean who takes direction to be a projection of temporal asymmetry), any two-way causal link is a cycle, and a cycle is an infinite descending chain — so *P1 itself* excludes it (`L0_of_P1`, v8.15: L0 is a theorem of P1; v8.14's separate "L0 burden" is withdrawn). The endless-future symmetry is answered by P1's presupposition of asymmetric causation, which is therefore part of what P1 costs.

### P4 — one fundamental being, the same at every world

*Rewritten in v8.10.* The v8.5 review called the coordination argument "undeveloped," and it was. The section now has three parts: what P4 does in the argument, which is less than the prose implied; the coordination argument developed as a trilemma for the second being; and the premise uniqueness actually rests on, which is one the argument already pays for elsewhere.

**Statement.** There is an N such that at every world N is fundamental (necessary, concrete, uncaused) and every fundamental thing is N. Lean: `P4_stmt : ∃ N, ∀ w, M.Fundamental w N ∧ ∀ t, M.Fundamental w t → t = N`.

**What P4 does, certified (v8.10).** `Core` is now `CoreNoP4` plus P4, and the types say which theorems use it. Over `CoreNoP4` alone: `no_modal_collapse`, `T2_4`, `T2_15`, `mental_of_nonactual`, `T3_7`–`T3_10`, and — with CE, NBL, CH, TR as explicit hypotheses — `mind_of_CE_NBL`: a necessary concrete mind exists whose state selects the actual first production and represents every alternative. Over `Core` (P4 needed): `T2_8` (one being brings about every world's reality), `identify`, `main`, `god_exists`, and the two originator theorems. So P4 does one job: it makes the necessary mind *unique* and the *same* being as the source and the agent. It does not make the mind exist. `W_P4.two_minds` displays what its denial gives: in the model with two fundamental beings N and N₂, each is necessary, concrete, uncaused at every world, and each is a mind. Denying P4 is not a naturalist exit. It is polytheism — or, on the other horn below, a bare second source with Exit 1's bill. The question P4 answers is *how many*, not *whether*.

**The case, developed: four positions for the second being.** Suppose N₁ and N₂ are both fundamental: necessary, concrete (each produces a first item at some world), uncaused. At a world where both produce, the first stage F_w contains items from each, and the joint reality — these N₁-items *with* these N₂-items, compossible, and law-governed together if they interact — is contingent, since each could have produced otherwise. CE applies to the joint contrast: which joint reality obtains is deterministic (no: `no_modal_collapse` holds over `CoreNoP4`), propensity-governed (no: III.7), or sensitive through a channel to a property of the candidate joint realities. By NBL the channel is a state of a thing; by CH its bearer produces in virtue of it; by TR it represents the joint alternatives — including what the *other* being contributes. Now the second being, N₂, is in one of three positions.

*(i) N₂ is a bare source.* No state of N₂ is sensitive to anything about the candidates; N₁'s state tracks the joint field and produces N₁'s part in view of the whole, accommodating whatever N₂ produces. CE is satisfied for the joint production — through N₁ — but N₂'s contribution is a contrast to which nothing bore: not N₂'s states (none are sensitive), not N₁'s (N₁ accommodates it; it does not produce it — CH gives N₁ causal responsibility only for N₁'s items). N₁'s accommodation explains the *fit*, not N₂'s output. So position (i) is one necessary mind plus one bare foundational source, and the bare source pays Exit 1's bill (III.1, D22) for its share of the first stage. Every additional fundamental being that is not itself field-sensitive is another such bill. *v8.14:* `W_Two.OneChannel.position_i` is this position — N₂ produces at every world, no state of N₂ is a channel, N₂ is an agent at no world — and in it IDF holds, CE holds, P4 fails, and **no world is `Bare`**. `Bare w := ¬Deterministic ∧ ¬Favored w ∧ ¬HasProp` (L:1007) is a predicate on worlds, satisfied nowhere once N₁'s state is a channel. "Pays Exit 1's bill for its share" is therefore prose with no formal referent: the file has no per-being notion of contribution, no premise a bare *source* violates, and no theorem that prices one. Position (i) is unpriced. **Priced (v8.17):** `BareOf w N₂` at every world and `¬ CEB` (`W_Two.OneChannel.position_i_priced`). Position (i) violates per-being CE, which the theist's one being satisfies as a theorem (`CEB_of_Axioms`).

*(ii) N₂ is field-sensitive.* Then N₂'s state, like N₁'s, tracks the joint field; by TR and FA N₂ is a mind; by P5 and P9 its representation is complete and accurate; by GG it is sensitive to maximality. So N₁ and N₂ are two necessary concrete beings, each with a complete accurate representation of every possible reality and the null alternative, each sensitive to the good, each producing its part in view of the whole. This is coherent — it is `W_Two.Both.position_ii` (v8.14; `W_P4` is position (iv), alternation, not this) — and it is the position the coordination argument has to answer. It is answered below, by identity, not by coordination; `W_Two.Both.not_IDF` certifies that IDF, and nothing else in the premise set, excludes it.

*(iii) N₂ is mediated.* N₂'s production is coordinated with N₁'s because N₁'s state is a cause-in-virtue-of of N₂'s items, or of N₂'s state. Then N₂'s items have N₁ as an ancestor and N₂ is not the being that originates them; and if N₂'s *state* is caused by N₁, N₂ is not uncaused in the sense that matters (its productive state depends on N₁). This is the case `N_anc_of_nec_concrete` describes once one fundamental is fixed: every other necessary concrete being is produced by it. Position (iii) is not a second fundamental being; it is P4⁺'s territory (III.8).

*(iv) N₁ and N₂ alternate.* *Added in v8.12; this is what the argument's own witness `W_P4` is, and the v8.10 text did not say so.* At each world exactly one of them produces and the other is idle: N₁ at w₀, N₂ at w′. Neither shares the other's necessary profile — N₁'s channel state is sensitive at w₀ only, N₂'s at w′ only — so IDF (below) is silent, and `W_P4.IDF_and_not_P4` certifies that IDF holds there while P4 fails. The alternation model is where the coordination question is sharpest, because there is no joint reality at any world to coordinate: the question is instead *which fundamental acts at w*. That contrast — N₁ rather than N₂ at w₀ — is contingent, and nothing bears on it: not N₁'s state (its sensitivity is to properties of the candidate realities, not to which being acts), not N₂'s (idle), and not the ground, which is identical across worlds. It is a bare contrast at the level of the actor, and it pays Exit 1's bill (D22) in its own currency: not "which reality" but "whose." A being whose essential sensitivity to the good operates at some worlds and not others, with nothing determining which, is at the idle worlds a bare source — position (i) — and at the active ones a mind. Alternation is (i) and (ii) taking turns, with a bare contrast added over which turn it is. *v8.14:* the same correction. In `W_P4` every productive world is `Favored` (CE is proved through `.state .s1` at `true` and `.state .s2` at `false`), so nothing is `Bare`; "a bare contrast at the level of the actor" is a contrast the model does not name and no premise addresses. Position (iv) is unpriced. **Priced (v8.17):** `BareOf true N₂ ∧ BareOf false N ∧ ¬ CEB` (`W_P4.alternation_priced`): an idle turn is a varying contribution to which nothing of the being's bears.

So the four positions leave, under per-being CE, exactly one live position for a genuine second fundamental producer: a second necessary mind (ii) — `contributors_are_minds` certifies that every fundamental that contributes is a mind, and `W_Two.Both.two_contributors_two_minds` that (ii) satisfies CEB. Positions (i) and (iv) violate CEB (`position_i_priced`, `alternation_priced`); their price is a premise the theist gets from `Axioms` for free (`CEB_of_Axioms`). "Denying P4 is polytheism, not naturalism" is now a theorem with its premise named: denying P4 *and keeping CEB* is polytheism; denying CEB is a second bare producer, and that is Exit 1's bill for its share — now formally: `BareOf`.

**What uniqueness rests on: identity of indiscernible fundamentals.** Position (ii) gives two beings with the same necessary nature: necessary, uncaused, concrete, bearing a necessary state that completely and accurately represents Ω ∪ {nil} and is sensitive to maximality, acting on the best reasons at w₀. What distinguishes N₁ from N₂? Not their outputs — those are contingent, vary across worlds, and cannot ground the identity of necessary beings (a necessary being's identity does not depend on what happens to obtain: that is the cross-world clause of P4, which is close to analytic). Not their states' contents — identical by P5, P9, GG. Only a *primitive thisness*: N₁ is N₁ and N₂ is N₂, and nothing makes it so. That is haecceitism at the foundation, and the argument has already met it: III.1, Objection 9, where the naturalist's haecceitistic exit was priced as necessitarianism about the constants plus primitive thisness. The uniqueness premise, stated at the level it actually operates, is therefore not "coordination requires a coordinator." It is **IDF — identity of indiscernible fundamentals**: fundamental beings that share every necessary feature are one. The coordination argument of v8.5 was gesturing at this: what "coordinates" two complete accurate maximality-tracking minds producing one reality is that there is nothing to make them two. IDF is the modal-realist's and the haecceitist's one point of disagreement with P4, and it is the same disagreement CE already has with them. The argument pays for anti-haecceitism at the foundation once, and P4 is what it buys the second time.

This is what the literature actually finds. Pruss and Rasmussen's *Necessary Existence* (2018) argues for at least one concrete necessary being and explicitly declines to argue uniqueness; Rasmussen's later route (2019) is through pure actuality — a foundation with no unrealized potential has no differentiating features, which is IDF under another description; Leftow (2012) secures uniqueness by GSA, that everything else derives from God, which is P4⁺'s route, and in "A Naturalist Cosmological Argument" gives the asymmetry that Oppy's necessary initial state has its necessity by position rather than by nature; Gellman (2000) runs a direct reductio of two necessary beings from total independence plus interaction, which is position (iii) rejected; Swinburne (1988, 2004) argues from simplicity, which Oppy meets with naturalist parsimony and "The Problem of Polytheisms" turns against him. Mghirbi (*IJPR* 2026) proves uniqueness and concedes in his own notes that it rests on a PSR-oriented conception of ultimate grounding and is "not a consequence of modal logic alone." The dossier agrees with the concession and locates the extra premise: IDF, not a stronger PSR.

**Objections.** (1) Island universes (Bigelow & Pargetter 1990; Bricker; Lewis 1986): two causally disconnected regions, each with its own necessary foundation, no joint reality to coordinate. (2) Plural fundamentals with parts that simply co-exist. (3) A being fundamental at one world but caused at another. (4) Oppy's naturalism: the fundamental being is the initial physical state — P4 holds, but the being is not the argument's. (5) The aggregate: the fundamental "being" is the plurality or fusion of N₁ and N₂, so P4 holds trivially and says nothing.

**Replies.** (1) Disconnection does not remove the joint contrast. A world with two disconnected regions has a contingent reality — *these* two regions — and CE asks what bore on it. If nothing in either foundation is sensitive to the joint field, both are bare (Exit 1, twice). If one is, it is a mind tracking a field that includes the other region, and the trilemma runs. If both are, IDF. Causal disconnection between the *regions* is not causal disconnection between the *foundations and the joint field*, and it is the latter that CE is about. `W_P4` is the model — its two beings produce at different worlds, which is the limiting case of disconnection — and it costs nothing formally; what it costs is what `W_P4.two_minds` shows: two necessary minds, or a bare source. (2) "Simply co-exist" is position (i), (ii), or (iv), and is answered as such. (3) A necessary being caused at some world depends there on something; if contingent, its necessity fails; if necessary, that is the fundamental and this was not. The cross-world clause is not where the dispute is. (4) Correct, and the argument agrees: Oppy's initial state *is* N as far as Tier 1 goes. The disagreement with Oppy is CE (III.1), not P4. (5) The aggregate move relocates the question without answering it: if the fusion is the fundamental, its parts are the beings whose productions must cohere, and the trilemma runs on the parts. If the parts have no productions of their own — only the fusion produces — then there is one producer, and P4 holds of it in the sense the argument needs. Nothing in the file's definitions distinguishes a simple N from a fusion whose parts do nothing; the argument is about the producer.

**Cost of denial (v8.17).** Position (ii): several necessary concrete minds — `contributors_are_minds` — with the same necessary profile, distinguished by primitive thisness alone (`W_Two.Both.position_ii`), excluded by IDF. Positions (i) and (iv): a second fundamental whose contribution is bare at some productive world — `W_Two.OneChannel.position_i_priced`, `W_P4.alternation_priced` — which is `¬CEB`, the per-being form of Exit 1. `W_P4.two_minds` remains a fact about `Mental := True` in that model; what makes a channel-less second fundamental a mind or not is CEB.

**Ceiling (v8.17).** Uniqueness is not a theorem of logic. Against a second fundamental producer it rests on two premises and one further claim. CEB (per-being CE) makes every contributor a mind and prices (i) and (iv). IDF excludes two contributors that share a necessary profile. What neither gives is that two channel-bearing contributors *do* share a profile: `SameProfile` demands matched necessary states with the same content, mentality, accuracy and sensitivity, and P5, P9, GG give each contributor *a* complete accurate maximality-sensitive state, not that their states match one-to-one. That claim — profile uniformity for necessary contributors — is the residue of III.6's "nothing to make them two", it is philosophical, and the dossier now says uniqueness is CEB + IDF + profile uniformity and nothing stronger. The formal work on P4 is finished; this is its honest shape.

### IDF — identity of indiscernible fundamentals

*Added in v8.12.* IDF has become load-bearing in three places — CE's haecceitistic exit (III.1, Objection 9), P4's uniqueness (above), and the token/type question in Tier 2 (D1, ID) — and it deserves its own entry rather than two replies.

**Statement.** Fundamental beings that share every necessary feature are one. Lean (optional, not a field of `Axioms`): `IDF_stmt : ∀ t t', (∀ w, Fundamental w t) → (∀ w, Fundamental w t') → SameProfile M t t' → t = t'`, where `SameProfile t t'` says that every necessary state of one is matched, in the other, by a necessary state with the same content, mentality, accuracy, and sensitivity profile, and conversely (Appendix A.3).

**What it is and what it is not.** IDF is the identity of indiscernibles restricted to fundamental beings and to their *necessary* features. It does not say that any two things sharing all properties are identical (Black's spheres are not fundamental beings); it does not say that two contingent things cannot be duplicates; it does not require Lewis's or anyone's global anti-haecceitism. It says one thing about one kind of thing: at the foundation, where a being's identity cannot rest on anything contingent — a necessary being's outputs vary across worlds and cannot individuate it — there is nothing left to separate two beings with the same necessary nature except a primitive thisness, and IDF denies that there is one.

**The case.** (a) A necessary being's identity is necessary; what individuates it must be necessary too; the only necessary features a fundamental has are those in its profile; so two fundamentals with the same profile have nothing necessary to distinguish them. (b) Primitive thisness at the foundation is exactly what CE's haecceitistic exit needs (III.1, Objection 9), and the argument has already priced that exit as necessitarianism about the qualitative content of physics plus a bare "this rather than that"; IDF was said (v8.12) to be "the same refusal, made once." *Corrected (v8.14):* it is not. Objection 9 is about tokens of the *first stage* — `ID_stmt : M.TypeDeterministic → M.Deterministic`, stated over `kind : Item → K`; IDF is about *fundamental things* — `SameProfile : Thing → Thing → Prop`. Different sorts, different premises, neither mentioned in the other; `W_P4` has IDF (certified) and ¬ID (`K := Unit` with both worlds productive). A haecceitist about contingent tokens who denies thisness for necessary beings holds ¬ID ∧ IDF consistently. IDF is a separate commitment with its own bill. (c) Rasmussen's pure-actuality route to uniqueness (2019) is IDF under another description — a foundation with no unrealized potential has no differentiating features — and Leftow's (2012) claim that a necessary being's necessity flows from its nature rather than its position is the premise that its *identity* does too.

**What IDF does not do, certified.** `W_P4.IDF_and_not_P4`: in the alternation model, two fundamentals with *different* profiles — one active at w₀, the other at w′ — satisfy IDF vacuously and violate P4. So IDF alone does not yield uniqueness. What closes the gap is horn (iv) above: a fundamental that is idle at a world is bare there, and alternation is a bare contrast over which being acts. Uniqueness therefore rests on IDF *together with* the pricing of bare sources by CE, and the dossier claims no more than that. (A per-being notion of "contribution to F_w" would let the four positions themselves be certified; the model does not have one, and adding it is the only remaining formal work on P4.) *v8.14:* "a fundamental that is idle at a world is bare there" is not the model's notion of `Bare` (world-indexed); see position (iv). What is certified: IDF excludes (ii) (`W_Two.Both.not_IDF`) and is silent on (i) and (iv). *v8.17:* the "per-being notion of contribution" this paragraph asked for exists (D29); with it, (i) and (iv) are priced by `¬CEB`, and IDF's remaining job is (ii).

**Objections.** (1) Haecceitism is a respectable position (Adams 1979; Kaplan); denying it globally is contested, and the argument cannot help itself to anti-haecceitism. (2) Two fundamentals might differ in a necessary feature the model does not represent — a primitive nature over and above states, content, and sensitivity. (3) Identity of indiscernibles fails for symmetric universes (Black 1952); a symmetric pair of foundations is the same case.

**Replies.** (1) The argument does not deny haecceitism globally; it denies primitive thisness for *fundamental* beings, and it has to, because the alternative is already on its books as Objection 9's price. A haecceitist who grants that contingent things may be individuated by thisness and denies it only at the foundation is not IDF's opponent; IDF's opponent must hold that the necessary ground of everything is individuated by nothing necessary. (2) Granted as a demand for care: `SameProfile` matches the model's necessary features (states, content, mentality, accuracy, sensitivity), and a skeptic who posits a further necessary feature — a primitive nature — owes an account of what it is such that two beings with identical states, identical content, identical sensitivity to the good, and identical outputs-in-every-world differ in it. That account, if it is not thisness under another name, is the account IDF asks for. (3) Black's spheres are contingent, individuated by their relations in a symmetric space; the foundation has no space to be symmetric in, and no contingent relations to be individuated by. The case IDF covers is the one Black's does not reach.

**Cost of denial.** Primitive thisness at the foundation: two or more necessary beings with the same complete accurate representation of every possibility and the same sensitivity to the good, distinguished by nothing. This is the polytheist's position (III.6, position (ii)), and the argument prices it as parsimony against a bare "this one, not that one" — the same bill CE's haecceitistic exit pays.

**Ceiling.** IDF cannot be proved against a committed haecceitist about the foundation. It can be shown to be the restricted, foundation-only form of a principle the argument needs anyway; to be what the pure-actuality and nature-not-position routes to uniqueness amount to; and — by `W_P4.IDF_and_not_P4` — to be not sufficient on its own, so that the dossier's uniqueness claim is honestly IDF plus CE's pricing of bare sources, and nothing stronger.

### Independent literature

Pruss, *The Principle of Sufficient Reason* (Cambridge, 2006), ch. 2; *Infinity, Causation, and Paradox* (Oxford, 2018). Koons, "A New Kalam Argument: Revenge of the Grim Reaper," *Noûs* 48 (2014). Schmid, "The End Is Near: Grim Reapers and Endless Futures," *Mind* 133 (2024). Schmid & Malpass, "Branching Actualism and Cosmological Arguments," *Philosophical Studies* 180 (2023). Meyer, "Explanatory Regresses and Non-Well-Founded Causation." Oppy, *Arguing About Gods* (Cambridge, 2006); "Uncaused Beginnings," *Faith and Philosophy* 27 (2010). Rowe, *The Cosmological Argument* (Princeton, 1975). Rasmussen, "From a Necessary Being to God," *IJPR* 66 (2009). Pruss & Rasmussen, *Necessary Existence* (Oxford, 2018). Pruss & Rasmussen, *Necessary Existence* (Oxford, 2018). Rasmussen, *How Reason Can Lead to God* (IVP, 2019); "From a Necessary Being to God," *IJPR* 66 (2009). Leftow, *God and Necessity* (Oxford, 2012); "A Naturalist Cosmological Argument." Gellman, "Prospects for a Sound Stage III of Cosmological Arguments," *Religious Studies* 36 (2000). Swinburne, "Could There Be More Than One God?" *Faith and Philosophy* 5 (1988); *The Existence of God*, 2nd ed. (Oxford, 2004). Mghirbi, "The Hierarchical Grounding Argument for a Necessary Being," *IJPR* 99 (2026). Bigelow & Pargetter, *Science and Necessity* (Cambridge, 1990). Bricker, "Island Universes and the Analysis of Modality," in Preyer & Siebelt eds., *Reality and Humean Supervenience* (2001). Adams, "Primitive Thisness and Primitive Identity," *J. Phil.* 76 (1979).
...
**The objection, at full strength (rewritten v8.13).** Non-Archimedean probability (NAP): Benci, Horsten & Wenmackers, "Non-Archimedean Probability" (2013) and "Infinitesimal Probabilities" (*BJPS* 2018), construct hyperreal-valued measures that are *regular* — every possible event has positive probability — totally ordered, and finitely additive, by taking limits along a non-principal ultrafilter over finite subsets. Under NAP the anti-chance argument fails at L2: every one of uncountably many kinds can carry a positive infinitesimal propensity, so no kind is assigned zero, and L4 is satisfied everywhere. The NAP theorist accepts what the construction requires — that the resulting measure is not translation-invariant, not conglomerable, and depends on the ultrafilter — and argues that regularity is worth those costs. v8.5 cited this; it did not argue against it.

**Replies.** The dossier argues Pruss's side, on four points, and says which are decisive for the foundation.

*(i) Underdetermination is not a cost the foundation can absorb.* The NAP measure is defined relative to a non-principal ultrafilter, and different ultrafilters give different measures — different infinitesimal values for the same events, and different orderings of events whose standard probabilities coincide. BHW grant this and treat the ultrafilter as a free parameter, on the model of choosing a coordinate system. For *credences* that may be tolerable: an agent may adopt one. For *objective propensities* — the objective chance of a one-off production — it is not: there is no fact in the ground that selects an ultrafilter, and a propensity that differs by ultrafilter is not a single propensity but a family, with nothing making one of them the chance. The NAP theorist at the foundation must therefore say either that the propensity is the whole family (then "the propensity of kind *k*" has no value, and `HasProp` has no content) or that N's nature fixes an ultrafilter — a non-constructive object no physical or metaphysical feature could distinguish from any other — which is an essential feature of N unconnected to anything. That is the count of III.4 (c) again: a second brute essential feature.

*(ii) Williamson's symmetry.* Williamson (2007): an infinite sequence of fair tosses; the event "all heads" and the event "all heads from toss 2" are isomorphic under shift, so a propensity that respects the symmetry gives them equal value; but the first is half the second; so both are 0, and no regular measure respects the symmetry. BHW accept that NAP is not shift-invariant. At the foundation the point bites harder: the space of kinds has no privileged enumeration, and an NAP propensity over it depends on one. A propensity over kinds that changes when the kinds are relabeled is not a propensity over kinds; it is a propensity over labels.

*(iii) Conglomerability and "too small" (Pruss 2012, 2014).* Infinitesimal propensities fail conglomerability: an event can have probability greater than each of its conditional probabilities across a partition. Pruss's "infinitesimals are too small" argument: in a countably infinite fair lottery, the events "ticket in the first *n*" have probability *n*·ε, each smaller than every positive real, yet their union is certain; the infinitesimal values cannot be the chances of anything that behaves like the lottery. Easwaran (2014) adds that hyperreal values are not the sort of thing a physical process could realize or a measurement distinguish: no frequency, no symmetry, no dynamics fixes an infinitesimal. For a one-off production with no repeatable structure, that objection has no answer — there is nothing at the foundation for the infinitesimals to be values *of*.

*(iv) What NAP buys, and what it costs, at the foundation.* NAP escapes L2 by giving every kind an infinitesimal weight. But then no kind has *appreciable* propensity: the actual kind obtained with an infinitesimal chance, and so did every alternative. The naturalist who takes this route has not explained the outcome by chance; he has assigned it a chance smaller than every positive real and called that an explanation. The difference between "obtained with infinitesimal propensity" and "obtained with nothing bearing on it" is a difference the NAP theorist must state, and stating it is the ultrafilter again.

**Cost of denial.** Hyperreal propensities at a one-off, individuated by an ultrafilter nothing in nature fixes; a propensity over kinds that is not invariant under relabeling the kinds; and an "explanation" of the outcome on which every alternative was equally infinitesimally likely. **Ceiling.** P11 cannot be proved against a committed non-Archimedean theorist. It can be shown to be the standard view; to be what every ordinary use of probability assumes; and — the point the dossier now presses — to be the only reading on which "propensity-governed" at the foundation names one propensity rather than an ultrafilter-indexed family. The NAP theorist has a developed position; its price at the foundation is (i). *Corrected (v8.14):* it is not true that the NAP literature "addresses credence" only. Benci, Horsten and Wenmackers' motivating cases (2013) are fair infinite lotteries and infinite coin-toss sequences presented as chance setups, and their 2018 paper replies explicitly to Williamson's shift-invariance argument and to Pruss's underdetermination, arguing that ultrafilter-relativity is a modelling choice of the same status as the choice of σ-algebra or of a measure in the real-valued case. One may reject that reply — the dossier does, on the ground that a one-off objective chance has no modelling context in which the choice is made — but it must be engaged, not denied to exist. Point (iv) above also needs care: assigning the actual kind a positive infinitesimal is precisely the NAP theorist's way of *avoiding* the probability-zero verdict; "explains nothing" is then a claim about explanation, not about NAP.
...
*CatU is not established by the constants of our laws, and the dossier withdraws any suggestion that it is.* Collapsing correlated constraints — the diproton, deuteron, neutron–proton mass difference, and Hoyle windows all trace to {m_u, m_d, Λ_QCD, α} — the honest count of independent sharply-bounded constraints is about five: the light-quark-mass island, Λ, Q, the low-entropy initial condition, and dimensionality. Five independent partitions yield finitely many kinds, and *within* each window variation is gradual. So the constants of our laws give a finite partition into sharply-separated kinds with continua inside them — not uncountably many kinds.

*What CatU and CatOpen jointly require, stated exactly (v8.13).* P12 needs uncountably many pairwise disjoint *open* regions, each containing a possible output. A second-countable topology has no such family (the countable chain condition), so CatU and CatOpen together say: the similarity topology on kinds is not second-countable — uncountably many kinds, each *isolated*, no kind a limit of others. That is a strong claim and the dossier states it as one. The v8.5 text's appeal to "the parameter space is a continuum within a family" does not deliver it: a continuum of parameters is a manifold, manifolds are second-countable, and on a manifold the kinds are points, not opens. Objection (1) below is right against that argument, and it is withdrawn.

*The positive argument (v8.13).* What delivers uncountably many isolated kinds is not continuous variation but *infinitely many independent discrete respects*. A law-structure is individuated by its discrete structural features: which fundamental field species it contains, what symmetry it has, what dimension, what topology. These are not continuously variable — a field species is present or absent, a quantum number is what it is (Dirac's point that particle species are not deformable into one another), a dimension is an integer — and a difference in any of them is a difference in kind: a law-structure with an additional field species is not a small perturbation of one without it, however many other features they share. So in the topology of *kinds*, a difference in any discrete structural respect is a discontinuity: no kind is a limit of kinds that differ from it in field content. That is CatOpen, and it is CatOpen for the right reason — not because a similarity metric happens to have gaps, but because kind-membership is fixed by discrete respects and discrete respects do not vary continuously. Now count. Nothing in physics or metaphysics bounds the number of possible field species: for any *n*, a law-structure with *n* independent species is possible (the Standard Model's seventeen are contingent; the landscape's vacua vary them; nothing forbids more). If, further, a law-structure with *infinitely many* independent species is possible — infinite towers are standard objects (Kaluza–Klein spectra), and the species-bound literature treats large and unbounded species numbers as physical questions, not incoherences — then the possible field contents are the subsets of a countably infinite menu, of which there are 2^ℵ₀. Each is its own isolated kind. That is CatU with CatOpen: uncountably many kinds, none a limit of the others.

*A second contestable premise (v8.14).* "A difference in field content is a difference in kind and not a limit" is a claim about the similarity topology, and standard physics supplies the opposite one. A field species with coupling ε → 0 *decouples*: every observable of the theory with the species converges to the observable of the theory without it. Under any similarity relation that tracks observables, dynamics, or coupling space, the law-structure without the species *is* the limit of the law-structures with it at small coupling. The positive argument therefore rests on categorical similarity in species content — CatOpen assumed for species, not argued — and Dirac's point (a given species' quantum numbers are not deformable) does not reach the presence or absence of a species with vanishing coupling. The dossier's "the only denial the positive argument leaves" is withdrawn: the naturalist's cheaper denial is coupling-space similarity, under which species-addition is a small change and the 2^ℵ₀ subsets are not isolated.

*Where the skeptic's exit is, exactly.* The argument requires possible law-structures with infinitely many independent discrete features. A skeptic who holds that every possible first stage is finitely specifiable — finite field content, finite information — gets countably many kinds (the finite subsets of the menu), and CatU fails. That is a real position, and it is the *only* denial of CatU that the positive argument leaves: not "kinds form a continuum" (that is ¬CatOpen and is answered by the discreteness of structural respects), but "no possible world has infinitely many independent structural features." Its cost is a modal thesis no physics supports — physics bounds species numbers by dynamics (Dvali's species bound lowers the gravitational cutoff; it does not forbid species), not by possibility — and it is a thesis about *every* possible law-structure, asserted from inside one.

*What the countable-kinds skeptic then faces (v8.13).* Suppose the kinds are countably many and isolated. P12 fails and `T2_15` does not fire, so `HasProp` is a live CE disjunct. The propensity over kinds is then one of two things. It is *uniform* — every kind equally weighted — in which case, being finitely additive over countably many isolated kinds, it gives each kind propensity zero (a uniform finitely additive measure on a countable set assigns zero to every singleton), and L4 says no kind can obtain: the actual kind obtained, so the uniform propensity is refuted by P11, **CatOpen** and L4, without CatU — CatOpen is needed because L4 speaks only of open regions, and the kind-classes are open only by it. *Certified in v8.14:* `uniform_countable_refuted`, from P11 + CatOpen + L4 via `L2_countable_uniform`. Or it is *non-uniform* — some kinds weighted more than others — in which case the weights are a structure of the ground that discriminates among the candidates, and the question is what they track. If they track a property of the kinds (simplicity, symmetry, anything), the production is sensitive to a property of the candidates through the ground's nomic structure: that is CE's third disjunct through the law channel, and NBL (III.2) says the law channel selects nothing — the weights are a selecting law. *Scoped (v8.14):* this is a reclassification the model does not support. `μ` and `Sens` are independent fields; a propensity whose weights track a property is, in the model, `HasProp` with a particular `μ`, and it is a `Sens` through the law channel only by a stipulation D17 does not make. The middle horn is prose in a stronger sense than the third: the file cannot state it. If they track nothing — the weights are what they are and answer to no property of the kinds — then the propensity is a discriminating essential feature of N unconnected to anything, which is the count of III.4 (c) once more, now at the level of the measure. So the countable-kinds route does not deliver a *bland* chance; it delivers a chance that is either refuted (uniform), a selecting law (property-tracking), or bare at the level of the weights. The uniform horn is certified (v8.14, `uniform_countable_refuted`); the other two are prose — the file has no notion of what a propensity's weights track, and the middle horn is a claim it cannot state.

*The empirical case against a propensity at the origin does not need CatU.* The natural (Liouville) measure over cosmological initial conditions does not normalize — the phase space is non-compact and the measure is infinite (Schiffrin & Wald 2012) — so the *canonical* measure gives `HasProp` nothing to be true with (v8.14: this is what Schiffrin & Wald show — that the Liouville/GHS measure supports no probabilistic claim about initial conditions — not that no measure exists). A naturalist who supplies a measure instead must defend it against the Boltzmann-brain problem (Carroll 2017), the Q-catastrophe (Garriga & Vilenkin), and, in the file, P11 and L4, which close any Archimedean propensity over the kind-partition. A naturalist who rejects all measures has taken Exit 1, rejecting at the foundation alone the measure cosmology uses for initial conditions everywhere else (III.1, cost (c)). Penrose's 10^(10^123) is a ratio of phase-space volumes under that measure and is not used as a probability anywhere in this argument; the qualitative fact it points to — an initial gravitational entropy near zero against a holographic maximum of about 2.6 × 10¹²² k_B (Egan & Lineweaver 2010) — is robust and is the fourth constraint above.
...
**Replies.** (1) Granted against the v8.5 argument from parameter continua, which is withdrawn. Against the positive argument it does not apply: the kinds there are individuated by discrete structural respects, and a space whose points differ in discrete respects is not a manifold in those respects. The manifold picture is the claim that every possible first stage is a small perturbation of every other across differences of field content, symmetry, and dimension; the conservative data answers this for the constants of our laws (the discontinuities are real), and the discreteness of structural respects answers it for law-structures. (2) If propensities are not defined over any topology on the alternatives, then the propensity account has no distribution to speak of, and "propensity-governed" has no content — which is not an exit from `T2_15` but a concession of it (HasProp is false), and it is what Schiffrin & Wald's non-normalizability delivers. (3) Granted as a demand for clarification: CatOpen does not say kinds are *automatically* open; it says a *similarity* topology, being about similarity, does not make outputs of one kind limits of outputs of another — and the data shows the kinds in question are separated by discontinuities in the physics, not by fiat. (4) This is the live objection to CatU, and the positive argument locates it exactly: it is the thesis that no possible law-structure has infinitely many independent discrete features. The dossier concedes its standing and prices it — a modal claim about every possible first stage with no physical support — and then follows it: with countably many isolated kinds, the propensity is uniform (refuted by P11 and L4), property-tracking (a selecting law, III.2), or bare at the weights (III.4's count). CatU's ceiling is here, and the countable-kinds skeptic's exit runs through the propensity fork.

**Cost of denial.** Of CatOpen: that a difference in field content, symmetry, or dimension is a small perturbation — against the physics and against the discreteness of the respects. Of CatU: that no possible law-structure has infinitely many independent discrete features — a modal thesis about every possible world with no physical support — followed by the propensity fork: a uniform chance refuted by L4, a property-tracking chance that is a selecting law, or bare weights. Of both: no propensity distribution at all, which is `¬HasProp` and Exit 1.

**Ceiling.** CatOpen has empirical support for the constants and a principled basis for law-structures (discrete respects do not vary continuously); its denial costs the physics of the boundaries. CatU has a positive argument (v8.13) — infinitely many independent discrete respects give 2^ℵ₀ isolated kinds — whose one contestable premise is that a law-structure with infinitely many independent features is possible. A skeptic who denies that has a real position; what the dossier now shows is that it does not buy a bland chance, because the propensity over countably many kinds is refuted, a selecting law, or bare at the weights. The anti-chance tier does not rest on CatU alone — non-normalizability closes `HasProp` for anyone using the cosmological measure, and P11/L4 close any Archimedean measure over an uncountable kind-partition — but the *formal* route through `P12_of_cat` does, and the propensity fork that catches the countable case is prose, not theorem. That is where a mathematically minded opponent should concentrate, and the dossier says what he will find.
...
|---|---|---|
| CE | That a skeptic who pays the bill errs; `W_Nat.witness` proves the exit consistent. **Also (v8.14):** that the theist's residue is "one yes/no" — `Maximal` is not unique and the only discriminating certified balance is defined by actuality (`W_Pref`) | Exit 1: the whole content of the first stage — five sharply-bounded constraints — with nothing bearing on any of it, against the theist's "which of the maximal class, and whether the inclination prevailed," a comparison the file does not size; the transition/boundary distinction is D3, a stipulation, not `T2_2`; Norton's dome is bare by the shape of the disjunction and its price is D17's exhaustiveness |
| NBL | That a nomic primitivist cannot posit a bearer-free selecting law | Nothing, unless every state channel is also off (`BothChannels.witness`, v8.14); the priced exit is the package `Axiarch`/`DLaw` — a law that chooses **and** no state that selects |
...
| GG, P8(a) | That a necessary being's essential inclination must track what it represents; and, at the formal level, that the balance is value rather than its inverse | Formally: a different `Q` in `Sens w Q (state s)` (`Toy.A` and `DState` differ in nothing else). The "count" of essential features is an interpretive premise asserting the belief–desire link the Smith reply denies (v8.14). Against Collins, "the bad is a reason for" is a price only given GG. The residue at w′ is shared |
| P8(b) | That a best reality exists | Replaced by P8s: good enough, not unbeaten |
| TR | That a fallible field-ranging inclination toward the best is content rather than a primitive modality. The fallible reading is now formalized with content about the state (`FallibleDir`, v8.17) and satisfied on both sides (`W_Pref.fallible_dir`, `W_Nat.fallible_dir`) | Certified: `manif` ties everything (`powers_all_tied`) or fails on a discriminating balance (`no_powers_at_maximal`); a fallible directedness is not a power (`fallible_not_power`), needs a discriminating balance and a losing world (`fallible_maximal_discriminates`), does not exist on a tied balance (`no_fallible_dir`), and is the theist's own modal profile. What remains is `Sens` — TR's content claim — priced by (a)–(c) and the Oderberg step, philosophical |
| P2 | That the Humean who withholds at the first items errs | Existence from nothing, or silence |
| P1 | That the unsatisfiable-pair theorist errs in calling the story's impossibility brute; **and** direction-neutral causation is excluded by L0, not P1 (v8.14) | An ungrounded chain of dependence whose every finite fragment is possible; or, for the direction-neutralist, the framework premise L0, which needs its own entry |
| P4 | Uniqueness, beyond CEB + IDF — profile uniformity for necessary contributors is not a theorem. **Certified (v8.17):** under CEB and no chance every fundamental contributor is a mind (`contributors_are_minds`); (i) and (iv) violate CEB (`position_i_priced`, `alternation_priced`); (ii) satisfies it and is excluded by IDF | Denying P4 with CEB: several necessary minds (polytheism). Denying CEB: a second producer whose contribution is bare — Exit 1's bill for its share, now formal (`BareOf`). The theist pays CEB nothing (`CEB_of_Axioms`) |
| IDF | That a fundamental being has no primitive thisness; and, certified, IDF alone is not P4 (`W_P4.IDF_and_not_P4`). IDF ≠ ID in both directions (`W_P4.IDF_and_not_ID`, `W_Two.Both.ID_and_not_IDF`). Its job after v8.17 is position (ii) alone, and it needs profile uniformity to reach it | Primitive thisness for necessary beings, paid separately from CE's Objection 9 |
| CEB | That a second fundamental's contribution cannot be bare — CEB is a theorem for the theist (`CEB_of_Axioms`) and a premise against the polytheist | A bare second producer (`W_Two.OneChannel.position_i_priced`) or an idle turn (`W_P4.alternation_priced`): per-being Exit 1 |
| P11 | That chance is Archimedean, against Benci–Horsten–Wenmackers — whose 2018 replies to ultrafilter-relativity and shift-invariance the dossier now reports (v8.14) | Hyperreal chance at a one-off with a modelling-relative ultrafilter; the dossier's reply is that a one-off has no modelling context, and that reply is philosophical |
| CatOpen | Little: sharp boundaries in kind are in the physics (D21) | Rejection of the discontinuities physics exhibits |
| CatU | That a law-structure has infinitely many independent features **and** that similarity is categorical in species content (v8.14: decoupling makes species-addition a limit) | Countably many kinds — then the propensity fork: uniform (certified refuted from P11 + **CatOpen** + L4, `uniform_countable_refuted`), property-tracking (`HasProp` with structure — a case the file cannot classify), bare weights (prose) |
...
### A.6 v8.14–8.15 additions: `naturalPowers`, `fallible_actual_iff`, the uniform horn, `L0_of_P1`, `inground_state_in_force`
...
(The witness models added in v8.14 — `Toy.god_holds`, `Toy.knows_everything`, `Toy.BothChannels`, `Toy.W_Pref.fork_is_residue`, `Toy.W_Two` — are in the file after `Toy.W_Nat`; like the other witnesses they are not reproduced here.)
...
### A.7 v8.17 additions: per-being contribution, CEB, `FallibleDir`
