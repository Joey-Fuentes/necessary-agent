...
## Version 8.7 — machine-checked core; P3, P6 and P10 derived; the necessary-mind conclusion needs no value premise; P4⁺ adopted: one being; soundness dossier complete; two naturalist packages certified (¬CE and ¬TR); the theist's residue certified and priced; the conclusion named (`God`) and certified (`god_exists`)
...
**Changes from v8.6 (this revision — v8.7: the conclusion named, the exits priced).**
...
- **D23** The TR fork (v8.7; open). `Sens w P (state s)` is primitive, so `NoTR` may stipulate it for a determinate-type disposition. D20 says such a disposition is one in which no alternatives figure, i.e. ¬CE, not ¬TR. The powers naturalist therefore faces a question the file does not settle: do the candidate realities bear on the outcome or not? If they do, the state ranges over them, and ranging over unproduced alternatives is what Oderberg (2017) calls specific indifference and reserves for the mental — TR follows. If they do not, `Sens` is false and the package is `W_Nat` under another name. Formalizing this requires a primitive `Directed s D` distinct from `Sens`, a witness that `Directed ∧ ¬Sens` is consistent, and an argument (not an axiom) that `Directed` with a discriminating D does not yield `Sens`. The nil alternative is the lever: a power directed at D is not directed at its own non-manifestation. Not yet formalized (III.5).
...
0. **The TR fork** (D23; III.5). Formalize `Directed s D` as a primitive distinct from `Sens`; certify `Directed ∧ ¬Sens` consistent; argue that `Directed` with a discriminating D does not yield `Sens`, so the powers naturalist must choose between TR (the candidates figure, the state ranges over them) and `W_Nat` (they do not). Answer the Dretskean modus tollens (III.5). This is the argument's logical load and its central open burden. If a referee produces field-sensitivity without abstraction, the conclusion weakens from "TR is forced" to "TR is the most economical reading" and must say so.
...
| TR | **The logical load of the argument.** Certified independent by a mindless, representation-free `NoTR` (v8.6): the powers naturalist's complete package. Not mild: the Molnar/Place/Oderberg physical-intentionality program holds that a disposition is directed at its manifestation-type without representing anything, and a consistent Dretskean runs FA as a modus tollens against TR. The fork in D23 is the reply; not yet formalized | The TR fork (open problem 0); an answer to the Dretskean modus tollens |
...
**To 1 (directedness is not content) — the fork.** This is the argument's central open burden (D23) and the reply is stated as a fork, not a refutation.

Take the powers naturalist's own description: the ground's state is directed at a determinate manifestation-type D, sensitive to D, causes a D-thing, represents nothing. Now ask the question the file does not settle: *do the candidate realities bear on the outcome, or not?*

*Horn 1: they do.* Then the state's contribution depends on how the candidates compare with respect to D — some have it, some lack it, and the production is sensitive to that. A state whose contribution depends on a comparison across a field of alternatives, most of which are not produced, is a state that in Oderberg's own vocabulary ranges over instances of a type with *specific indifference* — it is directed at "a D-thing," whichever, among the alternatives. Oderberg reserves that for the mental and says physical powers have no correlate of it. So on Horn 1, by the powers theorist's own criterion, the state has crossed the line: `Sens` holds, and the candidates figure, and figuring-across-a-field is abstraction, and abstraction is the mark of the mental. TR follows for this state, whatever is true of magnets.

*Horn 2: they do not.* Then the state is directed at D in the way a magnet is directed at north: no alternative figures; the power just does what it does. That is a coherent view of a disposition — it is Oderberg's view — but it is not an instance of `Sens` as D20 states its intended sense. A production in which no alternative figures is one to which nothing about the candidates bore. That is the *denial of CE*, and the package is `W_Nat` under another name, with Exit 1's bill (III.1).

So the powers naturalist must choose which premise he is denying. `NoTR` as filed takes Horn 1's `Sens` and Horn 2's representation-free state, and the fork says these do not go together: either the candidates figure (TR) or they do not (¬CE). What the fork does not yet do is *prove* that Horn 1's sensitivity entails abstraction. That requires (i) a primitive `Directed s D` distinct from `Sens`, (ii) a certified witness that `Directed ∧ ¬Sens` is consistent — the magnet — and (iii) an argument, in prose, that `Directed` with a discriminating D does not yield `Sens`, i.e. that a determinate-type power's directedness does not make the alternatives figure. None of that is an axiom; (iii) is the philosophical work, and it is where a referee will press. If a referee produces a model of field-sensitivity without abstraction — a state whose contribution ranges over the candidates but which, on some defensible criterion, does not abstract — then the fork fails, TR is not forced, and the conclusion must weaken to "TR is the most economical reading of a field-sensitive state," with the powers package standing as a live exit at the price of an unexplained field-sensitivity.
...
To deny TR is to hold that a state of the necessary ground through which the first production is sensitive to a property of the candidate realities, and in virtue of which the ground produces, has no alternative as content — including the alternative of producing nothing. `Toy.NoTR` certifies the position is consistent and mindless. Its price is the fork: either the candidates figure, in which case the state ranges over a field with specific indifference and the powers theorist's own criterion makes it mental, or they do not, in which case nothing about the candidates bore on the outcome and the position is Exit 1 with Exit 1's bill. The powers naturalist who holds that `Sens` is satisfied by a magnet-like directedness owes an account of *field-sensitivity without abstraction* — a state whose contribution depends on how unproduced alternatives compare, but which ranges over nothing. No such account is in the literature; Oderberg's is the closest, and it draws the line on the argument's side.

### Ceiling

TR cannot be proved against a theorist who insists that field-sensitivity is directedness without content, and the fork is stated as a burden, not a result: (i)–(iii) above are not yet formalized. What can be shown, and is: that TR asks for indicator content only; that the powers theorist's own criterion for the mental — specific indifference over a type's instances — is what a field-sensitive state exhibits; that the nil alternative is one no determinate-type power is directed at; that the Dretskean modus tollens is available only on Horn 2, which is ¬CE; and that `NoTR` as filed combines a sensitivity from Horn 1 with a state from Horn 2. The honest statement of where this leaves the powers naturalist: he has the cheaper of the two exits *if* field-sensitivity without abstraction can be made out, and the argument has not yet shown it cannot. That is the open problem ranked first in §10.
...
| TR | That field-sensitivity entails abstraction; the fork (D23) is stated, not formalized | Exit 2: field-sensitivity without abstraction — a state whose contribution depends on how unproduced alternatives compare but which ranges over nothing; or, on the other horn, Exit 1's bill. `NoTR.witness` proves the exit consistent and mindless |
...
**Two rows, not one.** v8.5 said the cheapest row was the only one the strongest opponent needs. That was wrong: there are two certified single-premise naturalist packages, and they belong to different opponents. Oppy's bare naturalist denies CE (`W_Nat`) and pays Exit 1's bill. The powers naturalist — anyone with a dispositional metaphysics of the initial state — denies TR (`NoTR`), keeps CE, and pays the fork: field-sensitivity without abstraction, or Exit 1 after all. The argument, at its most compressed, is therefore: **Tier 1 gives a necessary concrete originator, and `god_exists` follows if two things are granted — that something about the first production's outcome mattered, and that a state through which it mattered has the alternatives as content. The bare naturalist denies the first at the price of leaving the whole content of the first stage unexplained. The powers naturalist denies the second at the price of a sensitivity that ranges over a field without ranging over anything — or, if he cannot pay that, becomes the bare naturalist.** Whether either price is worth paying to avoid the conclusion is the judgment the argument is built to force; the conclusion itself, given the premises, is a theorem.

---

# Appendix A — Verbatim Lean definitions and premises

This appendix reproduces, unedited from `NecessaryAgent.lean`, the parts of the Lean file that Part I renders in words: the preliminaries and content type (A.0, added v8.7), the value-structure axiomatization behind P11 and its ℚ instance (A.1), the channel type, the model (§1.0) and the definitions (§1.1–1.6) (A.2), the premise statements (§2) and the premise structures (A.3), and — added in v8.7 per the external review — `KnowsAll`, the signatures of `identify` and `main`, and the definition of `God` with the signature of `god_exists` (A.4). A faithfulness check consists of reading each Lean line against the corresponding sentence of Part I. Everything else in the Lean file is proof, and is the kernel's business. The blocks are regenerated mechanically from the certified file at each version; the review's one discrepancy (A.1's `L2` signature dropping its `:= by`) is a deliberate abbreviation of a proof header.
