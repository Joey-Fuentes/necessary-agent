...
| P4 | `W_P4` — N with s1 produces c1 at `true`; N2 with s2 produces c2 at `false` | two fundamental beings | 2.8: one being brings about every reality — but `W_P4.two_minds`: both beings are necessary minds; denying P4 is polytheism, not naturalism |
...
1. **CatOpen and CatU** (P12 is now a theorem from them). CatOpen: supported empirically for the constants (D21) and principled for law-structures (discrete respects). CatU: a positive argument (v8.13, III.7) — infinitely many independent discrete structural respects give 2^ℵ₀ isolated kinds — with one contestable premise, that a law-structure with infinitely many independent features is possible. The countable-kinds skeptic is then caught by the *propensity fork* (uniform → refuted by L4; property-tracking → a selecting law; bare weights → III.4's count), which is prose. Remaining: certify the propensity fork, which needs a notion of what a propensity's weights track — not in the model.
...
6. **P4 — narrowed (v8.10).** Certified: the mind conclusion does not use P4 (`mind_of_CE_NBL` over `CoreNoP4`); its denial yields several necessary minds (`W_P4.two_minds`). III.6 develops the coordination argument as a trilemma for the second being and locates uniqueness at IDF — identity of indiscernible fundamentals — which is anti-haecceitism at the foundation, the commitment CE already needs against Objection 9. What remains is IDF's defense, which is III.1's, and parsimony against a polytheist who accepts primitive thisness. Not a naturalist's problem any longer.
...
17. **IDF** (v8.12). Stated as an optional premise with its own dossier entry (III.6). Certified not sufficient for P4 alone (`W_P4.IDF_and_not_P4`: alternation). Remaining: the only formal work left on uniqueness is a per-being notion of contribution to F_w, which would let the four positions of III.6 be certified rather than argued.
...
**Cost of denial.** Several necessary concrete minds, each with a complete accurate representation of every possible reality and sensitive to the good, distinguished by primitive thisness alone (position (ii) with ¬IDF); or a bare second source paying Exit 1's bill for its share of the first stage (position (i)); or haecceitism at the foundation, which is III.1's Objection 9 paid again. None of these is naturalism. `W_P4.two_minds` certifies the first.

**Ceiling.** Uniqueness is not a theorem of logic and the dossier does not present it as one. It rests on IDF, which is anti-haecceitism about the foundation: a commitment the argument already needs against CE's haecceitistic exit and prices there. What is certified (v8.10) is that this is *all* P4 is for — the necessary mind exists without it — and that its denial yields more necessary minds, not fewer. A skeptic who accepts primitive thisness at the foundation has a real position; it is polytheism, and the argument's claim over it is IDF plus parsimony, honestly labeled as such.
...
| GG, P8(a) | That a necessary being's essential inclination must track what it represents; and, at the formal level, that the balance is value rather than its inverse | A second essential feature unconnected to the first, with the unexplained relation between them (`DState`); or a theory of practical reason on which the bad is a reason for. The residue at w′ is shared, not added |
...
| P1 | That the unsatisfiable-pair theorist errs in calling the story's impossibility brute; and an answer for a theorist who denies causation has a direction | An ungrounded chain of dependence whose every finite fragment is possible; or backward causation, which P1 then forbids in the future story too |
| P4 | Uniqueness, beyond IDF (identity of indiscernible fundamentals) — not a theorem of logic; the mind conclusion does not need it (`CoreNoP4`) | Several necessary minds distinguished by primitive thisness (`W_P4.two_minds`), or a bare second source with Exit 1's bill, or haecceitism at the foundation (III.1, Obj. 9) — none of them naturalism |
| IDF | That a fundamental being has no primitive thisness; and, certified, IDF alone is not P4 (`W_P4.IDF_and_not_P4`: alternation) | Several necessary beings with identical necessary natures distinguished by nothing; or alternation, a bare contrast over which acts |
| P11 | That chance is Archimedean, against Benci–Horsten–Wenmackers | Hyperreal chance at a one-off: an ultrafilter nothing fixes, a propensity that changes when kinds are relabeled, and an outcome that was infinitesimally likely along with every alternative |
| CatOpen | Little: sharp boundaries in kind are in the physics (D21) | Rejection of the discontinuities physics exhibits |
| CatU | That a law-structure with infinitely many independent discrete features is possible | Countably many kinds — then the propensity fork: uniform (refuted by L4), property-tracking (a selecting law), or bare at the weights |
