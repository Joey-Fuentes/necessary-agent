...
*If you reject it:* Either you believe in infinitesimal chances at the origin of everything, or you believe all the possible ways reality could have begun are really one seamless spectrum with no breaks in kind (against the physics), or you believe something can happen even though there was zero tendency toward it.
...
The only way between the horns is a tendency toward the best that can *fail* — one that leans toward the best but sometimes doesn't get there. But the computer also confirms that this is not a tendency in the powers theorist's sense at all (his own rule says tendencies don't fail when unblocked). It's a lean toward one option among several, that sometimes loses. That is exactly what the argument means by "sensitive to the alternatives" — and a lean toward *whichever is best*, among options you haven't produced, is the thing the argument says only a mind has. So Exit 2 has one door left, and to take it you have to describe a lean toward the best, among alternatives, that isn't about any of them.

Two more things have now been checked. First, that kind of lean is free: any picture of the foundation can be described as "leaning toward whatever it actually produced" — the computer confirms the very same description fits the bare naturalist's picture and the theist's. So "the foundation leaned toward the best" says nothing by itself; the only thing that separates the two pictures is whether the alternatives *figured* in the lean. Second, none of the philosophers who allow tendencies to fail can supply this lean. On one account tendencies fail only when something interferes, and nothing interferes at the foundation. On another they fail by having a lower degree, but degree is a kind of chance, which the argument has already dealt with — or no degree, which is Exit 1. On a third, dispositions need a trigger, and the foundation has none. What's left is a lean toward the best among options it didn't produce, that sometimes loses, and that somehow isn't about any of those options. That is the one sentence the powers naturalist still has to write, and the argument's reply is that a lean toward *whichever* is best, across options you haven't made, is exactly the mark of a mind.
