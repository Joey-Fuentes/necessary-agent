...
Two things the earlier notes said were not checked by the computer now are. First, "a second being that merely produces" now has a price the computer can name: its contribution to the first stage is *bare* — it varies from world to world, no state of that being bears on it, and no chance governs it — and the premise that no fundamental being's contribution is bare (which the single-being picture gets for free) is exactly what it violates. Under that premise, every fundamental being that contributes anything is a mind; so denying uniqueness really is a choice between several minds and a second brute producer. Second, the "fallible lean toward the best" now has a definition with content — the state is the channel in every possible world, gets the best in the actual one, and misses it in some other — and the computer confirms it fits the theist's state and the bare naturalist's alike, cannot be a tendency that never fails, and cannot exist at all on a scale with no tilt. What separates the two sides is the one thing it does not mention: whether the options *figure* for the state. That is the premise the philosophical argument is about, and it is the only thing left.
